...
const char *g_szLookTaskToString[LOOK_MAX] = {
	"LOOK_NONE",   "LOOK_VECTOR", "LOOK_WAYPOINT",   "LOOK_WAYPOINT_NEXT_ONLY",
	"LOOK_AROUND", "LOOK_ENEMY",  "LOOK_LAST_ENEMY", "LOOK_HURT_ORIGIN",
	"LOOK_EDICT",  "LOOK_GROUND", "LOOK_SNIPE",      "LOOK_WAYPOINT_AIM",
	"LOOK_BUILD",  "LOOK_NOISE",
};
...
bool BotFunc_BreakableIsEnemy(edict_t *pBreakable, edict_t *pEdict)
{
	...
	if (!(flags & FL_WORLDBRUSH))
	{
		...
		if ((vSize.x >= vMySize.x) || (vSize.y >= vMySize.y)
...
void CBroadcastVoiceCommand ::execute(CBot *pBot)
{
	...
	if (pBot->isEnemy(m_pPlayer, false))
	{
		...
		if (pBot->wantToListen() && pBot->wantToListenToPlayerAttack(m_pPlayer)
		    && (pBot->distanceFrom(m_pPlayer) < CWaypointLocations::REACHABLE_RANGE))
...
void CBot ::runPlayerMove()
{
	...
	if (CClients::clientsDebugging(BOT_DEBUG_BUTTONS))
	{
		...
		sprintf(dbg, "m_pButtons = %d/%x, Weapon Select = %d, impulse = %d", cmd.buttons, cmd.buttons, cmd.weaponselect,
		        cmd.impulse);
...
bool CBot ::FVisible(edict_t *pEdict, bool bCheckHead)
{
	...
	if (bCheckHead || (pEdict == m_pEnemy) || CBotGlobals::isPlayer(pEdict))
	{
		...
		else if ( CClients::clientsDebugging(BOT_DEBUG_VIS) && (CBotGlobals::getTraceResult()->m_pEnt != nullptr) )
		{
		    ...
		    if ( edict )
		        debugoverlay->AddTextOverlay(CBotGlobals::getTraceResult()->endpos,2.0f,"Traceline hit
%s",edict->GetClassName());
		}
#endif */
		if (m_pEnemy == pEdict)
		{
...
void CBot ::think()
{
	...
	{
		m_pPlayerInfo = playerinfomanager->GetPlayerInfo(m_pEdict);
		logger->Log(LogLevel::INFO, "%s : m_pPlayerInfo = nullptr; Waiting for player info...", m_szBotName);
		return;
	}
	...
	{
#endif
		if (m_bWantToListen && !hasEnemy() && !hasSomeConditions(CONDITION_SEE_CUR_ENEMY)
		    && (m_fWantToListenTime < engine->Time()))
		{
...
bool CBot ::isOnLift()
{
	return ((m_vVelocity.z < -8.0f)
	        || (m_vVelocity.z >= 8.0f)); //&&(CClassInterface::getFlags(m_pEdict) & FL_ONGROUND);
}
...
void CBot ::spawnInit()
{
	...
	// m_pAvoidEntity = nullptr; // avoid this guy
	m_fLastWaypointVisible    = 0;
...
void CBot::setLastEnemy(edict_t *pEnemy)
{
	...
	pWpt = CWaypoints::getWaypoint(CWaypointLocations::NearestBlastWaypoint(m_vLastSeeEnemy, getOrigin(), 8192.0, -1,
...
void CBot ::findEnemy(edict_t *pOldEnemy)
{
	...
	/*else if ( CBotGlobals::entityIsAlive(pOldEnemy) ) /// lost enemy
	{
	    ...
	    pWpt =
	CWaypoints::getWaypoint(CWaypointLocations::NearestBlastWaypoint(m_vLastSeeEnemy,getOrigin(),8192.0,-1,true,true,false,false,0,false));
...
const char *pszConditionsDebugStrings[NUM_CONDITIONS] = { "CONDITION_ENEMY_OBSCURED",
	                                                      "CONDITION_NO_WEAPON",
...
{
	bool hastask = m_pSchedules->getCurrentTask() != nullptr;
	int iEnemyID = 0;
	...
	sprintf(msg, "Debugging bot: %s\n \
		...
		Enemy: %s (name = '%s')\n \
		---CONDITIONS---\n%s",
	        m_szBotName, (m_CurrentUtil >= 0) ? g_szUtils.at(m_CurrentUtil) : "none",
	        m_pSchedules->isEmpty() ? "none" : m_pSchedules->getCurrentSchedule()->getIDString(),
	        hastask ? task_string : "none", g_szLookTaskToString[m_iLookTask],
	        m_pNavigator->hasNextPoint() ? m_pNavigator->getCurrentWaypointID() : -1,
	        m_pNavigator->hasNextPoint() ? m_pNavigator->getCurrentGoalID() : -1, (m_fCurrentDanger / MAX_BELIEF) * 100,
	        (pEnemy != nullptr) ? pEnemy->GetClassName() : "none", (p != nullptr) ? p->GetName() : "none",
	        szConditions);
}
...
void CBot ::listenForPlayers()
{
	...
	for (register short int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if (wantToListenToPlayerFootsteps(pPlayer) && !isVisible(pPlayer)
		    && (m_bStatsCanUse && ((m_StatsCanUse.stats.m_iTeamMatesVisible == 0) /* && (m_fSeeTeamMateTime ...) */)))
		{
		...
		else if (bIsAttacking)
		{
			if (!bIsEnemy && wantToInvestigateSound())
			{
				...
				if (!m_pSchedules->isCurrentSchedule(SCHED_INVESTIGATE_NOISE)
				    && (randomFloat(0.0f, 0.75f) < m_pProfile->m_fBraveness))
				{
					...
					if (distanceFrom(vAttackerOrigin) > 96.0f)
					{
						...
						if ((TraceResult->fraction < 1.0f)
						    && (TraceResult->m_pEnt != m_pEdict->GetIServerEntity()->GetBaseEntity()))
						{
							...
							{
								// Facing my direction
								m_pSchedules->addFront(
								    new CBotInvestigateNoiseSched(m_vListenPosition, m_vListenPosition));
							}
							else
							{
								// Not facing my direction
								m_pSchedules->addFront(
								    new CBotInvestigateNoiseSched(vAttackerOrigin, m_vListenPosition));
							}
						}
					}
				}
	...
	if (moveToIsValid())
	{
		...
		if (m_pAvoidEntity && (m_fAvoidTime < engine->Time()))
		{
			if (canAvoid(m_pAvoidEntity))
			{
				...
				// m_vMoveTo = getOrigin() +
				// ((m_vMoveTo-getOrigin())-((m_vAvoidOrigin-getOrigin())*bot_avoid_strength.GetFloat())); float
				// fAvoidDist = distanceFrom(m_pAvoidEntity);
				...
				if (vMove.Length2D() > bot_avoid_strength.GetFloat())
				{
					...
					if (CClients::clientsDebugging(BOT_DEBUG_THINK))
					{
						...
						debugoverlay->AddLineOverlay(getOrigin(),
						                             m_bAvoidRight
						                                 ? (getOrigin() + (vLeft * bot_avoid_strength.GetFloat()))
						                                 : (getOrigin() - (vLeft * bot_avoid_strength.GetFloat())),
						                             0, 255, 0, false, 0.05f);
						debugoverlay->AddLineOverlay(
						    getOrigin(), getOrigin() + ((vMove / vMove.Length()) * bot_avoid_strength.GetFloat()), 255,
						    0, 0, false, 0.05f);
						debugoverlay->AddTextOverlayRGB(getOrigin() + Vector(0, 0, 100), 0, 0.05, 255, 255, 255, 255,
						                                "Avoiding: %s", m_pAvoidEntity.get()->GetClassName());
					}
#endif

					//*/
					// debugoverlay->AddLineOverlay (getOrigin(), m_vAvoidOrigin, 0,0,255, false, 0.05f);
					// debugoverlay->AddLineOverlay (getOrigin(), m_bAvoidRight ?
					// (getOrigin()+(vLeft*bot_avoid_strength.GetFloat())):(getOrigin()-(vLeft*bot_avoid_strength.GetFloat())),
					// 0,255,0, false, 0.05f); debugoverlay->AddLineOverlay (getOrigin(), m_vMoveTo, 255,0,0, false,
					// 0.05f);

					if (m_bAvoidRight)
						m_vMoveTo = getOrigin() + ((vMove / vMove.Length()) * bot_avoid_strength.GetFloat())
						          + (vLeft * bot_avoid_strength.GetFloat());
					else
						m_vMoveTo = getOrigin() + ((vMove / vMove.Length()) * bot_avoid_strength.GetFloat())
						          - (vLeft * bot_avoid_strength.GetFloat());
				}
...
bool CBot ::FInViewCone(edict_t *pEntity)
{
	...
	return (((origin - getEyePosition()).Length() > 1)
	        && (DotProductFromOrigin(origin) > 0)); // 90 degree !! 0.422618f ); // 65 degree field of view
}
...
Vector CBot::getAimVector(edict_t *pEntity)
{
	...
	else
	{
		...
		// check for QNAN
		if ((m_vAimOffset.x != m_vAimOffset.x) || (m_vAimOffset.y != m_vAimOffset.y)
	...
	if (CClients::clientsDebugging(BOT_DEBUG_AIM) && CClients::isListenServerClient(CClients::get(0)))
	{
		if (CClients::get(0)->getDebugBot() == getEdict())
		{
			...
			debugoverlay->AddTextOverlayRGB(m_vAimVector, line++, ftime, 255, 200, 100, 230, "fSensitivity = %0.2f",
			                                fSensitivity);
			debugoverlay->AddTextOverlayRGB(m_vAimVector, line++, ftime, 255, 200, 100, 230,
			                                "v_size = (%0.2f,%0.2f,%0.2f)", v_size.x, v_size.y, v_size.z);
			debugoverlay->AddTextOverlayRGB(m_vAimVector, line++, ftime, 255, 200, 100, 230,
			                                "v_desired_offset = (%0.2f,%0.2f,%0.2f)", v_desired_offset.x,
			                                v_desired_offset.y, v_desired_offset.z);
			debugoverlay->AddTextOverlayRGB(m_vAimVector, line++, ftime, 255, 200, 100, 230,
			                                "m_vAimOffset = (%0.2f,%0.2f,%0.2f)", m_vAimOffset.x, m_vAimOffset.y,
			                                m_vAimOffset.z);
		}
	}
#endif
                  ...
                  float fDist2D)
{
	...
	// target
	v_desired_offset->z +=
	    (fHeadOffset * m_pProfile->m_fAimSkill) + (randomFloat(0.0, 1.0f - m_pProfile->m_fAimSkill) * fHeadOffset);
}
...
Vector CBot::snipe(Vector &vAiming)
{
	if (m_fLookAroundTime < engine->Time())
	{
		...
		m_vLookAroundOffset =
		    Vector(randomFloat(-64.0f, 64.0f), randomFloat(-64.0f, 64.0f), randomFloat(-64.0f, 32.0f));
		// forward
...
void CBot ::getLookAtVector()
{
	...
	case LOOK_AROUND:
	{
		if (m_fLookAroundTime < engine->Time())
		{
			if ((m_fCurrentDanger < 10.0f)
			    || ((m_pNavigator->numPaths() == 0) || !m_pNavigator->randomDangerPath(&m_vLookAroundOffset)))
			{
				// random
			...
			m_vLookAroundOffset =
			    m_vLookAroundOffset + Vector(randomFloat(-128, 128), randomFloat(-128, 128), randomFloat(-16, 16));
		}
...
void CBot ::secondaryAttack(bool bHold)
{
	...
	{
		m_pButtons->holdButton(IN_ATTACK2, 0 /* reaction time? (time to press)*/, fHoldTime /* hold time*/,
...
void CBot ::primaryAttack(bool bHold, float fTime)
{
	...
	{
		m_pButtons->holdButton(IN_ATTACK, 0 /* reaction time? (time to press)*/, fHoldTime /* hold time*/,
		                       fLetGoTime /*let go time*/
		);
	}
}
...
#define SET_PROFILE_DATA_INT(varname, membername) \
	if (varname && *varname)                      \
	{                                             \
		pBotProfile->membername = atoi(varname);  \
	}
#define SET_PROFILE_STRING(varname, localname, membername) \
	if (varname && *varname)                               \
	{                                                      \
		localname = (char *)varname;                       \
	}                                                      \
	else                                                   \
	{                                                      \
		localname = pBotProfile->membername;               \
	}
...
#define CHECK_STRING(str) (((str) == nullptr) ? "nullptr" : (str))
