...
extern IVEngineServer
    *engine; // helper functions (messaging clients, loading content, making entities, running commands, etc)
...
class CBot
{
  public:
	...
	inline void clearFailedWeaponSelect()
	{
		m_iPrevWeaponSelectFailed = 0;
	}
	inline void failWeaponSelect()
	{
		m_iPrevWeaponSelectFailed++;
	}
	...
	inline float distanceFrom(edict_t *pEntity)
	{
		...
		return (pCollide->GetCollisionOrigin() - vMe).Length();
	}
	...
	CBotWeapon *getBestWeapon(edict_t *pEnemy, bool bAllowMelee = true, bool bAllowMeleeFallback = true,
	                          bool bMeleeOnly = false, bool bExplosivesOnly = false);
	...
	edict_t *getAvoidEntity()
	{
		return m_pAvoidEntity;
	}

	void setAvoidEntity(edict_t *pEntity)
	{
	...
	inline IBotNavigator *getNavigator()
	{
		return m_pNavigator;
	}
	...
	inline bool currentEnemy(edict_t *pEntity)
	{
		return m_pEnemy == pEntity;
	}
	...
	inline CBotSchedules *getSchedule()
	{
		return m_pSchedules;
	}
	...
	inline void selectWeapon(int iWeaponId)
	{
		m_iSelectWeapon = iWeaponId;
	}
	...
	inline void lookAtEdict(edict_t *pEdict)
	{
		m_pLookEdict = pEdict;
	}
	...
	inline void setAiming(Vector aiming)
	{
		m_vWaypointAim = aiming;
	}

	inline Vector getAiming()
	{
		return m_vWaypointAim;
	}

	inline void setLookVector(Vector vLook)
	{
		m_vLookVector = vLook;
	}

	inline Vector getLookVector()
	{
		return m_vLookVector;
	}

	inline void resetLookAroundTime()
	{
	...
	inline void wantToInvestigateSound(bool bSet)
	{
		m_bWantToInvestigateSound = bSet;
	}
	inline bool wantToShoot(void)
	{
		return m_bOpenFire;
	}
	inline void wantToShoot(bool bSet)
	{
		m_bOpenFire = bSet;
	}
	inline void wantToListen(bool bSet)
	{
		m_bWantToListen = bSet;
	}
	bool wantToListen();
	inline void wantToChangeWeapon(bool bSet)
	{
		m_bWantToChangeWeapon = bSet;
	}
	...
	{
		m_fWaypointTouchDistance = fDist;
	}

	inline float getTouchDistance()
	{
		return m_fWaypointTouchDistance;
	}

	inline CBotCmd *getUserCMD()
	{
		return &cmd;
	}
	...
	virtual bool overrideAmmoTypes()
	{
		return true;
	}
	...
	void setCoverFrom(edict_t *pCoverFrom)
	{
		m_pLastCoverFrom = MyEHandle(pCoverFrom);
	}
	...
	inline void resetAreaClear()
	{
		m_uSquadDetail.b1.said_area_clear = false;
	}
	...
	bot_statistics_t *getStats()
	{
		if (m_bStatsCanUse)
			return &m_StatsCanUse;
		return nullptr;
	}
	...
	eBotAction getCurrentUtil(void)
	{
		return m_CurrentUtil;
	}
...
class CBots
{
  public:
	...
	static void setMaxBots(int iMax)
	{
		m_iMaxBots = iMax;
	}

	static int getMaxBots()
	{
		return m_iMaxBots;
	}

	static void setMinBots(int iMin)
	{
		m_iMinBots = iMin;
	}

	static int getMinBots()
	{
		return m_iMinBots;
	}
	...
	static CBot *get(int iIndex)
	{
		return m_Bots[iIndex];
	}
	static CBot *get(edict_t *pPlayer)
	{
		return m_Bots[slotOfEdict(pPlayer)];
	}
