...
void CBotTF2::spawnInit()
{
	...
	m_pNonPVSRocket        = MyEHandle(nullptr);
...
void CBotTF2::addKnownEnemyTeleporter(edict_t *pTele)
{
	...
	CTeamFortress2Mod::updateFocusPoint(CBotGlobals::entityOrigin(pTele), true, false);
}
...
void CBotTF2::addKnownEnemyDispenser(edict_t *pDisp)
{
	...
	CTeamFortress2Mod::addTeamKnownDispenser(CBotGlobals::entityOrigin(pDisp));
}
...
void CBotTF2::modThink()
{
	...
	// Non-PVS projectile scan: detect rockets/pipes outside PVS (behind bot, etc.)
	if (m_NearestEnemyRocket == nullptr && m_pNearestPipeGren.get() == nullptr)
	{
		static float fProjScanTime = 0.0f;
		if (fProjScanTime < engine->Time())
		{
			Vector vOrigin = getOrigin();
			static const char *projectiles[] = {
			    "tf_projectile_rocket", "tf_projectile_sentryrocket",
			    "tf_projectile_pipe", "tf_projectile_pipe_remote",
			};
			for (int i = 0; i < 4; i++)
			{
				edict_t *pProj = CClassInterface::FindEntityByClassnameNearest(
				    vOrigin, projectiles[i], 512.0f, nullptr);
				if (pProj && CBotGlobals::entityIsValid(pProj)
				    && CTeamFortress2Mod::isHostileProjectile(
				           pProj, CTeamFortress2Mod::getEnemyTeam(m_iTeam))
				    && incomingRocket(512.0f))
				{
					m_pNonPVSRocket = pProj;
					m_fStrafeTime    = 0.0f; // force immediate dodge check
					break;
				}
			}
			fProjScanTime = engine->Time() + 0.25f;
		}
	}
	...
	// Also avoid visible enemy teleporters
	if (m_pNearestEnemyTeleporter.get() != nullptr
	    && !(m_iClass == TF_CLASS_SPY && (isDisguised() || isCloaked()))
	    && !CTeamFortress2Mod::TF2_IsPlayerInvuln(m_pEdict))
	{
		edict_t *pTele = m_pNearestEnemyTeleporter.get();
		if (CBotGlobals::entityIsValid(pTele) && CBotGlobals::entityIsAlive(pTele)
		    && isVisible(pTele) && pTele != m_pAttackingEnemy.get()
		    && distanceFrom(pTele) < 512.0f)
		{
			Vector vPos = CBotGlobals::entityOrigin(pTele);
			Vector vAway = getOrigin() - vPos;
			vAway.z = 0;
			if (vAway.Length() > 0.1f)
			{
	...
	// Also avoid visible enemy dispensers
	if (m_pNearestEnemyDisp.get() != nullptr
	    && !(m_iClass == TF_CLASS_SPY && (isDisguised() || isCloaked())))
	{
		edict_t *pDisp = m_pNearestEnemyDisp.get();
		if (CBotGlobals::entityIsValid(pDisp) && CBotGlobals::entityIsAlive(pDisp)
		    && isVisible(pDisp) && pDisp != m_pAttackingEnemy.get()
		    && distanceFrom(pDisp) < 384.0f)
		{
			Vector vPos = CBotGlobals::entityOrigin(pDisp);
			Vector vAway = getOrigin() - vPos;
			vAway.z = 0;
			if (vAway.Length() > 0.1f)
			{
	...
	// Also avoid known enemy teleporter positions
	for (auto &h : m_KnownEnemyTeleporters)
	{
		edict_t *pKnown = h.get();
		if (!pKnown || !CBotGlobals::entityIsValid(pKnown)
		    || !CBotGlobals::entityIsAlive(pKnown)) continue;
	...
	// Also avoid known enemy dispenser positions
	for (auto &h : m_KnownEnemyDispensers)
	{
		edict_t *pKnown = h.get();
		if (!pKnown || !CBotGlobals::entityIsValid(pKnown)
		    || !CBotGlobals::entityIsAlive(pKnown)) continue;
	...
	case TF_CLASS_DEMOMAN:
		if (m_iTrapType != TF_TRAP_TYPE_NONE)
		{
			if (m_pEnemy)
			{
				if ((CBotGlobals::entityOrigin(m_pEnemy) - m_vStickyLocation).Length()
				    < BLAST_RADIUS * 1.5f)
				{
					// Scottish Resistance: aim before detonating for selective blast
					CBotWeapon *pSticky = m_pWeapons->getWeapon(
					...
					if (pSticky)
					{
						...
						if (pEnt && CClassInterface::TF2_getItemDefinitionIndex(pEnt) == 130)
							setLookVector(CBotGlobals::entityOrigin(m_pEnemy));
					}
					detonateStickies();
				}
			}
	...
	case TF_CLASS_HWGUY:
	{
		...
		if (bRevMiniGun)
		{
			...
			if ((m_fRevMiniGunTime + m_fNextRevMiniGunTime) > engine->Time())
			{
				...
				if (m_fIdealMoveSpeed > 110.0f)
					m_fIdealMoveSpeed = 110.0f; // human-like revved speed
	...
	case TF_CLASS_SPY:
		...
		// If no visible building to sap, check personal known lists
		if (!m_pSchedules->hasSchedule(SCHED_SPY_SAP_BUILDING)
		    && m_fSpySapTime < engine->Time())
		{
			edict_t *pBest   = nullptr;
			float fBestDist  = 2048.0f;

			for (auto &h : m_KnownEnemyTeleporters)
			{
				edict_t *pTel = h.get();
				if (!pTel || !CBotGlobals::entityIsValid(pTel)
				    || !CBotGlobals::entityIsAlive(pTel)) continue;
				if (CTeamFortress2Mod::isTeleporterSapped(pTel)) continue;
				float fD = distanceFrom(pTel);
				if (fD < fBestDist) { fBestDist = fD; pBest = pTel; }
			}

			for (auto &h : m_KnownEnemyDispensers)
			{
				edict_t *pDis = h.get();
				if (!pDis || !CBotGlobals::entityIsValid(pDis)
				    || !CBotGlobals::entityIsAlive(pDis)) continue;
				if (CTeamFortress2Mod::isDispenserSapped(pDis)) continue;
				float fD = distanceFrom(pDis);
				if (fD < fBestDist) { fBestDist = fD; pBest = pDis; }
			}

			if (pBest && fBestDist < 2048.0f)
			{
				int iEnemy = CTeamFortress2Mod::getEnemyTeam(m_iTeam);
				eEngiBuild eType = ENGI_TELE;
				if (CTeamFortress2Mod::isDispenser(pBest, iEnemy))
					eType = ENGI_DISP;
				m_fSpySapTime = engine->Time() + randomFloat(0.5f, 1.5f);
				m_pSchedules->freeMemory();
				m_pSchedules->add(new CBotSpySapBuildingSched(pBest, eType));
			}
		}
...
void CBotTF2::handleWeapons()
{
	    ...
	    && wantToShoot() && (bTankClose || isVisible(m_pEnemy)) && isEnemy(m_pEnemy) && m_pWeapons)
	{
		...
		// Demoman: occasionally use sticky launcher instead of grenades (human-like alternation)
		if (m_iClass == TF_CLASS_DEMOMAN && pWeapon
		    && pWeapon->getID() == TF2_WEAPON_GRENADELAUNCHER)
		{
			CBotWeapon *pSticky = m_pWeapons->getWeapon(
			    CWeapons::getWeapon(TF2_WEAPON_PIPEBOMBS));
			if (pSticky && pSticky->hasWeapon() && !pSticky->outOfAmmo(this)
			    && randomFloat(0.0f, 1.0f) < 0.3f)
				pWeapon = pSticky;
		}

		setLookAtTask(LOOK_ENEMY);

		// Self-damage guard: switch explosive to melee when too close
		if ((m_iClass == TF_CLASS_SOLDIER || m_iClass == TF_CLASS_DEMOMAN)
		    && pWeapon && pWeapon->isExplosive() && m_pEnemy
		    && distanceFrom(m_pEnemy) < 128.0f)
		{
			CBotWeapon *pMelee = m_pWeapons->getBestWeapon(m_pEnemy, true, true, true);
			if (pMelee && pMelee->hasWeapon())
				pWeapon = pMelee;
		}
		...
		// Wall self-splash check: don't fire explosives when a wall is too close
		if (pWeapon && pWeapon->isExplosive() && !pWeapon->isMelee()
		    && m_pEnemy && distanceFrom(m_pEnemy) < 200.0f)
		{
			Vector vEyes   = getEyePosition();
			Vector vTarget = CBotGlobals::entityOrigin(m_pEnemy.get());
			Vector vAim    = vTarget - vEyes;
			float fDist    = vAim.Length();
			if (fDist > 0.1f)
			{
				vAim = vAim / fDist;
				CTraceFilterWorldAndPropsOnly filter;
				CBotGlobals::traceLine(vEyes, vEyes + vAim * 200.0f,
				                       MASK_SOLID_BRUSHONLY, &filter);
				trace_t *tr = CBotGlobals::getTraceResult();
				if (tr && tr->fraction < 1.0f)
				{
					Vector vAway = getOrigin() - tr->endpos;
					vAway.z      = 0;
					if (vAway.Length() > 0.1f)
						setMoveTo(getOrigin()
						    + vAway / vAway.Length() * 128.0f);
					return; // back away, skip firing
				}
			}
		}
		...
		if (fDist < fRange)
		{
			...
			if (CClassInterface::getVelocity(pRocket, &vel))
			{
				...
				if (fSpeed > 0.1f)
				{
					...
					// Jarate/Mad Milk arc through the air — apply gravity
					const char *pszClass = pRocket->GetClassName();
					if (pszClass
					    && (strcmp(pszClass, "tf_projectile_jar") == 0
					        || strcmp(pszClass, "tf_projectile_jar_milk") == 0))
					{
						float fTime      = fDist / fSpeed;
						float fGravity   = sv_gravity.IsValid()
						    ? sv_gravity.GetFloat() : 800.0f;
						vcomp.z -= 0.5f * fGravity * fTime * fTime;
					}
				}
				else
					vcomp = vorg;
		...
		if (fDist < fRange)
		{
			...
			if (CClassInterface::getVelocity(pRocket, &vel))
			{
				...
				if (fSpeed > 0.1f)
				{
					...
					float fTime = fDist / fSpeed;
					vcomp.z -= 0.5f * sv_gravity.GetFloat() * fTime * fTime;
				}
				else
					vcomp = vorg;
			}
...
bool CBotFortress::dodgeProjectile()
{
	if (m_fStrafeTime > engine->Time())
		return false;

	// Spy: don't dodge while cloaked — movement betrays position
	if (m_iClass == TF_CLASS_SPY && isCloaked())
	{
		m_fStrafeTime = engine->Time() + 0.3f;
		return false;
	...
	// Crossfire detection: rotate dodge 90° if moving toward a second projectile
	    ...
	    && distanceFrom(pSecond) < 800.0f)
	{
		...
		if (vToBot2.Length2D() > 0.1f && vDodge.Length2D() > 0.1f)
		{
			...
			if (vDodge.Dot(vToBot2) < -0.5f) // angle > 120°
			{
	...
	if (fLen > 0.1f)
	{
		...
		{
			int iOldPrio = m_iMoveLookPriority;
			setMoveLookPriority(MOVELOOK_EVENT);
			setMoveTo(vDest);
			setMoveLookPriority(iOldPrio);
...
void CBotTF2::getTasks(unsigned int iIgnore)
{
	...
	// Pick a teleporter target: visible first, then known, then team-shared
	edict_t *pTeleTarget = m_pNearestEnemyTeleporter.get();

	if (!pTeleTarget && !m_KnownEnemyTeleporters.empty())
	{
		float fBestDist = 9999.0f;
		for (auto &h : m_KnownEnemyTeleporters)
		{
	...
	if (!pTeleTarget)
	{
		for (int i = 0; i < 8; i++)
		{
			if (CTeamFortress2Mod::m_fTeamKnownTeleporterTimes[i] > engine->Time())
			{
				Vector vKnown = CTeamFortress2Mod::m_vTeamKnownTeleporters[i];
				float fD = (vKnown - getOrigin()).Length();
				if (fD < 2000.0f)
				{
					int iEnemy = CTeamFortress2Mod::getEnemyTeam(m_iTeam);
					edict_t *pCheck = CClassInterface::FindEntityByClassnameNearest(
					    vKnown, "obj_teleporter", 400.0f, nullptr);
					if (pCheck && CBotGlobals::entityIsValid(pCheck)
					    && CBotGlobals::entityIsAlive(pCheck)
					    && CTeamFortress2Mod::getTeam(pCheck) == iEnemy)
					{
	...
	// Pick a dispenser target: visible first, then known, then team-shared
	edict_t *pDispTarget = m_pNearestEnemyDisp.get();

	if (!pDispTarget && !m_KnownEnemyDispensers.empty())
	{
		float fBestDist = 9999.0f;
		for (auto &h : m_KnownEnemyDispensers)
		{
	...
	if (!pDispTarget)
	{
		for (int i = 0; i < 8; i++)
		{
			if (CTeamFortress2Mod::m_fTeamKnownDispenserTimes[i] > engine->Time())
			{
				Vector vKnown = CTeamFortress2Mod::m_vTeamKnownDispensers[i];
				float fD = (vKnown - getOrigin()).Length();
				if (fD < 2000.0f)
				{
					int iEnemy = CTeamFortress2Mod::getEnemyTeam(m_iTeam);
					edict_t *pCheck = CClassInterface::FindEntityByClassnameNearest(
					    vKnown, "obj_dispenser", 400.0f, nullptr);
					if (pCheck && CBotGlobals::entityIsValid(pCheck)
					    && CBotGlobals::entityIsAlive(pCheck)
					    && CTeamFortress2Mod::getTeam(pCheck) == iEnemy)
					{
	...
	if ((pSentryTarget != nullptr) && !CTeamFortress2Mod::TF2_IsPlayerInvuln(m_pEdict))
	{
		...
		// Nest recognition: count buildings near the sentry for threat escalation
		int iNestCount = 1;
		Vector vSentryPos = CBotGlobals::entityOrigin(pSentryTarget);
		for (auto &h : m_KnownEnemyTeleporters)
		{
			edict_t *pOther = h.get();
			if (pOther && CBotGlobals::entityIsValid(pOther) && CBotGlobals::entityIsAlive(pOther)
			    && (CBotGlobals::entityOrigin(pOther) - vSentryPos).Length() < 400.0f)
				iNestCount++;
		}
		for (auto &h : m_KnownEnemyDispensers)
		{
		...
		{
			edict_t *pOther = h.get();
			if (pOther && pOther != pSentryTarget
			    && CBotGlobals::entityIsValid(pOther) && CBotGlobals::entityIsAlive(pOther)
			    && (CBotGlobals::entityOrigin(pOther) - vSentryPos).Length() < 400.0f)
				iNestCount++;
		}
		if (iNestCount >= 3)
		    ...
		    && m_iClass != TF_CLASS_SPY && pWeapon && !pWeapon->outOfAmmo(this))
		{
			...
			if (m_iClass == TF_CLASS_DEMOMAN || m_iClass == TF_CLASS_SOLDIER
			    || m_iClass == TF_CLASS_HWGUY) fTeleClassMult = 1.5f;
			else if (m_iClass == TF_CLASS_PYRO || m_iClass == TF_CLASS_SCOUT)
				fTeleClassMult = 0.8f;
			else fTeleClassMult = 0.5f;

			float fDist = distanceFrom(pTeleTarget);
			float fTeleUtil = (500.0f / (fDist + 1.0f)) * fTeleClassMult;
			if (iNestCount >= 3)
				fTeleUtil *= 1.5f;
			ADD_UTILITY_DATA(BOT_UTIL_ATTACK_TELEPORTER, true, fTeleUtil,
			                 ENTINDEX(pTeleTarget));
		}
		    ...
		    && (!m_pEnemy || distanceFrom(pDispTarget) < 400.0f))
		{
			...
			if (iNestCount >= 3)
				fDispUtil *= 1.5f;
			ADD_UTILITY_DATA(BOT_UTIL_ATTACK_DISPENSER, true, fDispUtil,
			                 ENTINDEX(pDispTarget));
		}
...
void CBotTF2::detonateStickies(bool isJumping)
{
	...
	CBotWeapon *pSticky = m_pWeapons->getWeapon(CWeapons::getWeapon(TF2_WEAPON_PIPEBOMBS));
	float fArmTime = 0.8f; // default sticky launcher
	if (pSticky)
	{
		edict_t *pEnt = pSticky->getWeaponEntity();
		if (pEnt && CClassInterface::TF2_getItemDefinitionIndex(pEnt) == 130)
                                               ...
                                               // *pWaypointHealth, CWaypoint *pWaypointAmmo )
{
	...
	break;
	case BOT_UTIL_ATTACK_TELEPORTER:
	{
	...
	break;
	case BOT_UTIL_ATTACK_DISPENSER:
	{
		edict_t *pDisp = INDEXENT(util->getIntData());
		if (pDisp && CBotGlobals::entityIsValid(pDisp) && CBotGlobals::entityIsAlive(pDisp))
			m_pSchedules->add(new CBotTF2AttackSentryGun(pDisp, m_pWeapons->getPrimaryWeapon()));
	}
	break;
                                 ...
                                 float fDistance, bool bCycledIn)
{
	...
	if (m_bStickyCharging)
	{
		...
		// Quickiebomb (item 1150): charges ~2.5x faster than stock
		edict_t *pStickyEnt = pStickyLauncher->getWeaponEntity();
		if (pStickyEnt && CClassInterface::TF2_getItemDefinitionIndex(pStickyEnt) == 1150)
			fCharge *= 0.4f;
	...
	if (pWeapon)
	{
		...
		for (int i = 1; i <= CBotGlobals::maxClients(); i++)
		{
			...
			if (m_iClass == TF_CLASS_SOLDIER || m_iClass == TF_CLASS_DEMOMAN)
			{
				...
				// Fire charged sticky whenever sticky launcher is the active weapon
				if (pStickyLauncher && pStickyLauncher->hasWeapon()
				    && !pStickyLauncher->outOfAmmo(this) && pStickyLauncher->getClip1(this) > 0
		...
		if (m_iClass == TF_CLASS_DEMOMAN)
		{
			...
			// Cycle sticky launcher into combat every 6-10s (burst mode)
			else if (bStickyReady && pStickyLauncher->getClip1(this) >= 3
			    && fDistance >= 300.0f
			    && fDistance < pStickyLauncher->getPrimaryMaxRange()
			    && m_fNextStickyCycle < engine->Time()
			    && !m_bStickyCharging && m_fStickyDetTime == 0.0f)
			{
				// Start a burst: fire 3 charged stickies in quick succession, then detonate
				...
				// Release charged burst sticky when charge duration expires
				static bool bBurstReleased = false;
				if (m_bStickyCharging && m_fStickyBurstChargeEnd < engine->Time() && !bBurstReleased)
				{
					m_pButtons->letGo(IN_ATTACK);
					m_bStickyCharging = false;
					bBurstReleased = true;
				...
				if (m_iStickyBurstCount > 0 && m_fStickyBurstTime < engine->Time()
				    && getCurrentWeapon() == pStickyLauncher)
				{
					// Compute charge proportional to distance, QB-aware
					edict_t *pStickyEnt = pStickyLauncher->getWeaponEntity();
					int iItem = pStickyEnt
					    ? CClassInterface::TF2_getItemDefinitionIndex(pStickyEnt) : 0;
					float fBurstCharge = max(0.15f,
					    (fDistance / pStickyLauncher->getPrimaryMaxRange()) * 0.8f);
					if (iItem == 1150) // Quickiebomb charges faster
						fBurstCharge *= 0.4f;

					primaryAttack(true); // hold to charge
					m_bStickyCharging = true;
					m_fStickyBurstChargeEnd = engine->Time() + fBurstCharge;
					m_fStickyBurstTime = engine->Time() + fBurstCharge + 0.1f;
					bBurstReleased = false;
					m_iStickyBurstCount--;
					...
					if (m_iStickyBurstCount == 0)
					{
						...
						// Track where the stickies actually land
						Vector vAim = CBotGlobals::entityOrigin(m_pEnemy.get()) - getOrigin();
						if (vAim.Length2D() > 0.1f)
						{
							vAim.z = 0;
							vAim.NormalizeInPlace();
							m_vStickyLocation = getOrigin()
							    + vAim * (700.0f * fBurstCharge);
						}
						else
							m_vStickyLocation = CBotGlobals::entityOrigin(m_pEnemy.get());
					}
				...
				else if (m_fStickyDetTime < engine->Time())
				{
					...
					if (fEnemyToSticky < BLAST_RADIUS && fSelfToSticky > (BLAST_RADIUS / 2))
					{
						tapButton(IN_ATTACK2);
						m_fStickyDetTime = 0.0f;
						if (pGrenadeLauncher && pGrenadeLauncher->hasWeapon()
						    && !pGrenadeLauncher->outOfAmmo(this)
						    && getCurrentWeapon() != pGrenadeLauncher)
							select_CWeapon(pGrenadeLauncher->getWeaponInfo());
					}
					else
						m_fStickyDetTime = engine->Time() + 0.1f;
