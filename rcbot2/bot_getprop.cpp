 ...
 * @return 		Server class matching the name, or nullptr if none found.
 */
 ...
 * @return 		SendProp pointer on success, nullptr on failure.
 */
...
void CClassInterface::init()
{
	...
	DEFINE_GETPROP(GETPROP_TF2SPYDISGUISED_TARGET, "CTFPlayer", "m_hDisguiseTarget", 0);
	...
	DEFINE_GETPROP(GETPROP_TF2_OBJTR_m_bCPCapRateScalesWithPlayers, "CTFObjectiveResource",
...
bool CClassInterface ::getTF2ObjectiveResource(CTFObjectiveResource *pResource)
{
	...
	pResource->m_bCPCapRateScalesWithPlayers =
	    g_GetProps[GETPROP_TF2_OBJTR_m_bCPCapRateScalesWithPlayers].getBoolPointer(edict);
...
edict_t *CClassInterface::FindEntityByClassnameNearest(Vector vstart, const char *classname, float fMindist,
                                                       edict_t *pOwner)
{
	edict_t *current;
