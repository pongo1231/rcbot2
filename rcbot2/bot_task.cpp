...
void CBotTFEngiTankSentry :: execute (CBot *pBot,CBotSchedule *pSchedule)
{
    ...
    if ( !CBotGlobals::entityIsAlive(m_pEnemy) )
        complete();
    else if ( !CBotGlobals::entityIsAlive(m_pSentry) || !CBotGlobals::entityIsValid(m_pSentry) ||
!CTeamFortress2Mod::isSentry(m_pSentry) ) fail(); else
    {
        Vector vOrigin;
        Vector vComp;
...
void CBotTF2DemomanPipeEnemy ::execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0)
	{
		...
		if (vOtherWaypoint.z > (m_vEnemy.z + 32.0f))
		{
			...
			m_vAim.z =
			    m_vEnemy.z
			    + getGrenadeZ(pBot->getEdict(), m_pEnemy, pBot->getOrigin(), m_vEnemy,
