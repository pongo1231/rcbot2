...
typedef enum
{
	...
	BOT_UTIL_DESTROY_NEST,
	BOT_UTIL_ATTACK_TELEPORTER,
	BOT_UTIL_ATTACK_DISPENSER,
	...
	BOT_UTIL_MVM_COLLECT_CASH,
	BOT_UTIL_MVM_MEDIC_REVIVE,
	BOT_UTIL_SAP_MVM_ROBOT,
	BOT_UTIL_ATTACK_TANK,
	BOT_UTIL_SPY_REDISGUISE,
	BOT_UTIL_SPY_INFILTRATE,
	BOT_UTIL_SPY_LURK,
	BOT_UTIL_BUILDSENTRY_COMBAT,
	BOT_UTIL_MAX
} eBotAction;

const extern std::unordered_map<int, const char *> g_szUtils;

class CBotUtility
{
  public:
	CBotUtility(CBot *pBot, eBotAction id, bool bCanDo, float fUtil, CBotWeapon *pWeapon = nullptr, int iData = 0,
	...
	inline CBotWeapon *getWeaponChoice()
	{
		return m_pWeapon;
	}
...
#define ADD_UTILITY_WEAPON_DATA_VECTOR(utilname, condition, utility, weapon, data, vector)      \
	if (m_fUtilTimes[utilname] < engine->Time())                                                \
	{                                                                                           \
		if (condition)                                                                          \
		{                                                                                       \
...
#define ADD_UTILITY_DATA_VECTOR(utilname, condition, utility, data, vector)                      \
	...
	{                                                                                            \
		if (condition)                                                                           \
		{                                                                                        \
			utils.addUtility(CBotUtility(this, utilname, true, utility, nullptr, data, vector)); \
		}                                                                                        \
	}
...
#define ADD_UTILITY_DATA(utilname, condition, utility, data)                             \
	...
	{                                                                                    \
		if (condition)                                                                   \
		{                                                                                \
			utils.addUtility(CBotUtility(this, utilname, true, utility, nullptr, data)); \
		}                                                                                \
	}
