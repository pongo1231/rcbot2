...
void CBotVisibles ::checkVisible(edict_t *pEntity, int *iTicks, bool *bVisible, int &iIndex, bool bCheckHead)
{
	...
	*bVisible = false;
	...
	{
		// if ( CClients::clientsDebugging() && CClients::get(0)->isDebuggingBot(m_pBot) &&
		// (ENTINDEX(pEntity)<CBotGlobals::maxClients()) )
		...
		if (m_pBot->FInViewCone(pEntity))
		{
			...
			*iTicks      = *iTicks + 1;
			...
			playerInPVS = engine->CheckOriginInPVS(vEntityOrigin, m_bPvs,
			                                       sizeof(m_bPvs)); // engine->CheckBoxInPVS( vectorSurroundMins,
			                                                        // vectorSurroundMaxs, m_bPvs, sizeof( m_bPvs ) );
				...
				*bVisible = m_pBot->FVisible(pEntity, bCheckHead);
				...
				if (*bVisible)
				{
					if (CClients::clientsDebugging(BOT_DEBUG_VIS)
					    && CClients::get(0)->isDebuggingBot(m_pBot->getEdict())
					    && (ENTINDEX(pEntity) <= CBotGlobals::maxClients()))
				...
				else
				{
					if (CClients::clientsDebugging(BOT_DEBUG_VIS)
					    && CClients::get(0)->isDebuggingBot(m_pBot->getEdict())
					    && (ENTINDEX(pEntity) <= CBotGlobals::maxClients()))
						debugoverlay->AddTextOverlayRGB(CBotGlobals::entityOrigin(pEntity), 0, 0.1, 255, 0, 0, 200,
						                                "INVISIBLE");
				}
#endif
			}
			// else if ( CClients::clientsDebugging() && CClients::get(0)->isDebuggingBot(m_pBot) &&
			// (ENTINDEX(pEntity)<CBotGlobals::maxClients()))
