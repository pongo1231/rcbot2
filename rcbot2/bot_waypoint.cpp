...
CWaypoint *CWaypointNavigator ::chooseBestFromBeliefBetweenAreas(std::vector<AStarNode *> &goals, bool bHighDanger,
                                                                 bool bIgnoreBelief)
{
	...
	case 0:
		return nullptr;
	case 1:
		return CWaypoints::getWaypoint(goals[0]->getWaypoint());
	default:
	{
		AStarNode *node;
...
// best waypoints are those with lowest danger
CWaypoint *CWaypointNavigator ::chooseBestFromBelief(std::vector<CWaypoint *> &goals, bool bHighDanger,
                                                     int iSearchFlags, int iTeam)
{
	CWaypoint *pWpt = nullptr;
	CWaypoint *pCheck;
...
void CWaypointNavigator ::belief(Vector vOrigin, Vector vOther, float fBelief, float fStrength, BotBelief iType)
{
	static float factor;
	static float fEDist;
	...
	iWptFrom = CWaypointLocations::NearestWaypoint(vOrigin, 2048.0, -1, true, true, false, nullptr, false, 0, false,
	                                               true, vOther);
	iWptTo = CWaypointLocations::NearestWaypoint(vOther, 2048.0, -1, true, true, false, nullptr, false, 0, false, true,
	                                             vOrigin);
	...
	for (size_t i = 0; i < m_iVisibles.size(); i++)
	{
		...
		else if (iType == BELIEF_DANGER)
		{
			...
			// debugoverlay->AddTextOverlayRGB(pWpt->getOrigin(),0,5.0f,255,0,0,200,"Danger
			// %0.2f",m_fBelief[iWptIndex]);
		}
	}
	...
	for (size_t i = 0; i < m_iInvisibles.size(); i++)
	{
		...
		else if (iType == BELIEF_SAFETY)
		{
			...
			// debugoverlay->AddTextOverlayRGB(pWpt->getOrigin(),1,5.0f,255,0,0,200,"Danger INV
			// %0.2f",m_fBelief[iWptIndex]);
		}
	}
...
bool CWaypointNavigator ::getHideSpotPosition(Vector vCoverOrigin, Vector *vCover)
{
	...
	if (m_pBot->hasGoal())
		iWpt =
		    CWaypointLocations::GetCoverWaypoint(m_pBot->getOrigin(), vCoverOrigin, nullptr, m_pBot->getGoalOrigin());
	else
		iWpt = CWaypointLocations::GetCoverWaypoint(m_pBot->getOrigin(), vCoverOrigin, nullptr);
...
float CWaypointNavigator ::distanceTo(Vector vOrigin)
{
	...
	if (m_iCurrentWaypoint == -1)
		m_iCurrentWaypoint =
		    CWaypointLocations::NearestWaypoint(m_pBot->getOrigin(), CWaypointLocations::REACHABLE_RANGE, -1, true,
		                                        false, true, nullptr, false, m_pBot->getTeam());

	if (m_iCurrentWaypoint != -1)
	{
		iGoal = CWaypointLocations::NearestWaypoint(vOrigin, CWaypointLocations::REACHABLE_RANGE, -1, true, false, true,
                                    ...
                                    int iGoalId, int iConditions, int iDangerId)
{
	if (bRestart)
	{
		...
		*bFail          = false;
		...
		if (iGoalId == -1)
			m_iGoalWaypoint =
			    CWaypointLocations::NearestWaypoint(vTo, CWaypointLocations::REACHABLE_RANGE, m_iLastFailedWpt, true,
			                                        false, true, &m_iFailedGoals, false, m_pBot->getTeam());
		else
		...
		bool bIgnore =
		    m_pBot->getIgnoreBox(&vIgnore, &fIgnoreSize) && (pGoalWaypoint->distanceFrom(vFrom) > (fIgnoreSize * 2));

		m_iCurrentWaypoint = CWaypointLocations::NearestWaypoint(
		    vFrom, CWaypointLocations::REACHABLE_RANGE, m_iLastFailedWpt, true, false, true, nullptr, false,
		    m_pBot->getTeam(), true, false, vIgnore, 0, nullptr, bIgnore, fIgnoreSize);
		...
		if (m_iCurrentWaypoint == -1)
		{
			...
			m_iCurrentWaypoint = CWaypointLocations::NearestWaypoint(
			    vFrom, CWaypointLocations::REACHABLE_RANGE, -1, true, false, true, nullptr, false, m_pBot->getTeam(),
			    false, false, Vector(0, 0, 0), 0, m_pBot->getEdict());
	...
	while (!bFoundGoal && !m_theOpenList.empty() && (iLoops < iMaxLoops))
	{
		...
		// can get here now
		m_iFailedGoals.erase(std::remove(m_iFailedGoals.begin(), m_iFailedGoals.end(), iCurrentNode),
		                     m_iFailedGoals.end());
		...
		for (iPath = 0; iPath < iMaxPaths; iPath++)
		{
			...
			if (rcbot_debug_show_route.GetBool())
			{
				...
				{
					debugoverlay->AddLineOverlayAlpha(succWpt->getOrigin(), currWpt->getOrigin(), 255, 0, 0, 255, false,
					                                  5.0f);
				}
			}
#endif
			...
			{
				if ((m_pBot->getEnemy() != nullptr) && CBotGlobals::isPlayer(m_pBot->getEnemy())
	...
	while ((iCurrentNode != -1) && (iCurrentNode != m_iCurrentWaypoint) && (iLoops <= iNumWaypoints))
	{
		...
		if (rcbot_debug_show_route.GetBool())
		{
			...
			if (!engine->IsDedicatedServer() && ((pListenEdict = CClients::getListenServerClient()) != nullptr))
			{
				debugoverlay->AddLineOverlayAlpha(CWaypoints::getWaypoint(iCurrentNode)->getOrigin()
				                                      + Vector(0, 0, 8.0f),
				                                  CWaypoints::getWaypoint(iParent)->getOrigin() + Vector(0, 0, 8.0f),
				                                  255, 255, 255, 255, false, 5.0f);
			}
		}
#endif
...
{
	int iwpt =
	    CWaypointLocations::NearestWaypoint(vOrigin, 100, -1, true, false, true, nullptr, false, m_pBot->getTeam());
...
void CWaypointNavigator ::rollBackPosition()
{
	...
	m_iCurrentWaypoint =
	    CWaypointLocations::NearestWaypoint(m_vPreviousPoint, CWaypointLocations::REACHABLE_RANGE, m_iLastFailedWpt,
	                                        true, false, true, nullptr, false, m_pBot->getTeam());
...
void CWaypointNavigator ::updatePosition()
{
	...
	if (!m_bWorkingRoute)
	{
		bool movetype_ok = CClassInterface::isMoveType(m_pBot->getEdict(), MOVETYPE_LADDER)
		                || CClassInterface::isMoveType(m_pBot->getEdict(), MOVETYPE_FLYGRAVITY);

		// bTouched = false;

		bTouched =
		    pWaypoint->touched(m_pBot->getOrigin(), m_vOffset, m_pBot->getTouchDistance(), !m_pBot->isUnderWater());
		...
		if (bTouched)
		{
			...
			if (stats)
			{
				if ((stats->stats.m_iEnemiesVisible > stats->stats.m_iTeamMatesVisible)
				    && (stats->stats.m_iEnemiesInRange > 0))
			...
			else
			{
				...
				if (pWaypoint)
				{
					...
					// The waypoint height is probably the Z axis, so I created a Vector with 0 for xy and waypoint
					// height for z
					if (iWaypointFlagsPrev & CWaypointTypes::W_FL_TELEPORT_CHEAT)
						CBotGlobals::teleportPlayer(m_pBot->getEdict(),
						                            pWaypoint->getOrigin()
						                                - Vector(0, 0, (CWaypoint::WAYPOINT_HEIGHT / 2)));
				}
				if (m_iCurrentWaypoint != -1)
...
void CWaypoint ::drawPathBeam(CWaypoint *to, unsigned short int iDrawType)
{
	...
	case DRAWTYPE_EFFECTS:
		g_pEffects->Beam(m_vOrigin, to->getOrigin(), CWaypoints::waypointTexture(), 0, 0, 1, 1, PATHWAYPOINT_WIDTH,
		                 PATHWAYPOINT_WIDTH, 255, 1, r, g, b, 200, 10);
		break;
#ifndef __linux__
...
void CWaypoint ::draw(edict_t *pEdict, bool bDrawPaths, unsigned short int iDrawType)
{
			...
			if (distanceFrom(CBotGlobals::entityOrigin(pEdict)) < fDistance)
			{
				...
				if (m_iFlags)
				{
					if (pCurrentMod->isWaypointAreaValid(m_iArea, m_iFlags))
						debugoverlay->AddTextOverlayRGB(m_vOrigin + Vector(0, 0, fHeight + 4.0f), 0, 1, 255, 255, 255,
						                                255, "%d", m_iArea);
					else
						debugoverlay->AddTextOverlayRGB(m_vOrigin + Vector(0, 0, fHeight + 4.0f), 0, 1, 255, 0, 0, 255,
						                                "%d", m_iArea);
				}
				...
				if (CClients::clientsDebugging())
				{
					...
					if (pClient)
					{
						...
						if (pBot)
						{
							debugoverlay->AddTextOverlayRGB(
							    m_vOrigin + Vector(0, 0, fHeight + 8.0f), 0, 1, 0, 0, 255, 255, "%0.4f",
							    pBot->getNavigator()->getBelief(CWaypoints::getWaypointIndex(this)));
						}
					}
				}
	...
	case DRAWTYPE_DEBUGENGINE:
...
#ifndef __linux__
		// draw waypoint
		debugoverlay->AddLineOverlay(m_vOrigin - Vector(0, 0, fHeight), m_vOrigin + Vector(0, 0, fHeight), r, g, b,
		                             false, 1);

		// draw aim
		debugoverlay->AddLineOverlay(m_vOrigin + Vector(0, 0, fHeight / 2),
		                             m_vOrigin + Vector(0, 0, fHeight / 2) + vAim * 48, r, g, b, false, 1);
		...
		break;
	case DRAWTYPE_EFFECTS:
		g_pEffects->Beam(m_vOrigin - Vector(0, 0, fHeight), m_vOrigin + Vector(0, 0, fHeight),
		                 CWaypoints::waypointTexture(), 0, 0, 1, 1, WAYPOINT_WIDTH, WAYPOINT_WIDTH, 255, 1, r, g, b, a,
		                 10); //*/

		/*g_pEffects->Beam( m_vOrigin + Vector(0,0,fHeight/2), m_vOrigin + Vector(0,0,fHeight/2) + vAim*48
		   CWaypoints::waypointTexture(), 0, 0, 1, 1, WAYPOINT_WIDTH/2, WAYPOINT_WIDTH/2, 255, 1, r, g, b, a, 10);//*/
		break;
	}
	...
	for (register short int i = 0; i < iSize; i++)
	{
		if (pWpt->isUsed() && pWpt->hasFlag(iWptFlag))
		{
			...
			if (pair.pEdict != nullptr)
			{
				...
				CBotGlobals::traceLine(vOrigin, vOrigin - Vector(0, 0, CWaypointLocations::REACHABLE_RANGE),
...
// pPlayer is the person who called the command to save, nullptr if automatic
...
bool CWaypoints ::load(const char *szMapName)
{
	...
	if (szMapName == nullptr)
		CBotGlobals::buildFileName(filename, CBotGlobals::getMapName(), BOT_WAYPOINT_FOLDER, BOT_WAYPOINT_EXTENSION,
		                           true);
	else
...
{
	int iWpt = CWaypointLocations::GetCoverWaypoint(vPlayerOrigin, vPinchOrigin, nullptr, &vPinchOrigin);
                                   ...
                                   unsigned char *failedwpts)
{
	...
	float distance;
	int iwpt  = -1;
	int iFrom = CWaypointLocations::NearestWaypoint(vOrigin, fDist, -1, true, false, true, nullptr, false, iTeam);
	...
	for (i = 0; i < size; i++)
	{
		...
		if (pWpt->isUsed() && pWpt->forTeam(iTeam))
		{
			...
			{
				// BUG FIX for DOD:S
				if (!pCurrentMod->isWaypointAreaValid(
				        pWpt->getArea(),
				        iFlags)) // CTeamFortress2Mod::m_ObjectiveResource.isWaypointAreaValid(pWpt->getArea()) )
					continue;
...
int CWaypoints ::addWaypoint(CClient *pClient, const char *type1, const char *type2, const char *type3,
                             const char *type4, bool bUseTemplate)
{
	int iFlags           = 0;
...
int CWaypoints ::addWaypoint(edict_t *pPlayer, Vector vOrigin, int iFlags, bool bAutoPath, int iYaw, int iArea,
                             float fRadius)
{
...
{
    CWaypoint *pRunTo =
CWaypoints::getNextCoverPoint(CWaypoints::getWaypoint(m_iCurrentWaypoint),CWaypoints::getWaypoint(iId)) ;
	...
	for (int i = 0; i < numWaypoints(); i++)
	{
		if (m_theWaypoints[i].isUsed() && (m_theWaypoints[i].getFlags() > 0))
		{
			if ((m_theWaypoints[i].getArea() > iNumCps)
			    || (bAutoFixNonArea && (m_theWaypoints[i].getArea() == 0)
			        && m_theWaypoints[i].hasSomeFlags(CWaypointTypes::W_FL_SENTRY | CWaypointTypes::W_FL_DEFEND
			                                          | CWaypointTypes::W_FL_SNIPER | CWaypointTypes::W_FL_CAPPOINT
			                                          | CWaypointTypes::W_FL_TELE_EXIT)))
			{
				m_theWaypoints[i].setArea(
				    CTeamFortress2Mod::m_ObjectiveResource.NearestArea(m_theWaypoints[i].getOrigin()));
				CBotGlobals::botMessage(nullptr, 0, "Changed Waypoint id %d area to (area = %d)", i,
				                        m_theWaypoints[i].getArea());
			}
		}
	}
}
...
void CWaypoints ::checkAreas(edict_t *pActivator)
{
	...
	for (int i = 0; i < numWaypoints(); i++)
	{
		if (m_theWaypoints[i].isUsed() && (m_theWaypoints[i].getFlags() > 0))
		{
			if (m_theWaypoints[i].getArea() > iNumCps)
			{
				CBotGlobals::botMessage(pActivator, 0, "Invalid Waypoint id %d (area = %d)", i,
				                        m_theWaypoints[i].getArea());
			}
		}
	}
...
CWaypoint *CWaypoints ::randomWaypointGoalNearestArea(int iFlags, int iTeam, int iArea, bool bForceArea, CBot *pBot,
                                                      bool bHighDanger, Vector *origin, int iIgnore, bool bIgnoreBelief,
                                                      int iWpt1)
{
	register short int i;
	static short int size;
...
CWaypoint *CWaypoints ::randomWaypointGoalBetweenArea(int iFlags, int iTeam, int iArea, bool bForceArea, CBot *pBot,
                                                      bool bHighDanger, Vector *org1, Vector *org2, bool bIgnoreBelief,
                                                      int iWpt1, int iWpt2)
{
	register short int i;
	static short int size;
...
CWaypoint *CWaypoints ::randomWaypointGoal(int iFlags, int iTeam, int iArea, bool bForceArea, CBot *pBot,
                                           bool bHighDanger, int iSearchFlags, int iIgnore)
{
	register short int i;
	static short int size;
	CWaypoint *pWpt;
...
{
	return (m_iFlags
	        & (CWaypointTypes::W_FL_DEFEND | CWaypointTypes::W_FL_ROCKET_JUMP | CWaypointTypes::W_FL_DOUBLEJUMP
	           | CWaypointTypes::W_FL_SENTRY | // or machine gun (DOD)
	           CWaypointTypes::W_FL_SNIPER | CWaypointTypes::W_FL_TELE_EXIT | CWaypointTypes::W_FL_TELE_ENTRANCE))
	     > 0;
}
...
void CWaypointTypes ::setup()
{
	...
	addType(new CWaypointType(W_FL_AMMO, "ammo", "bot can sometimes get ammo here", WptColor(50, 100, 10),
	                          (1 << MOD_TF2) | (1 << MOD_HLDM2)));
	addType(new CWaypointType(W_FL_RESUPPLY, "resupply", "TF2 bot can always get ammo and health here",
	                          WptColor(255, 100, 255), (1 << MOD_TF2)));
	...
	addType(new CWaypointType(W_FL_DOUBLEJUMP, "doublejump", "TF2 scout can double jump here", WptColor(10, 10, 100),
	                          (1 << MOD_TF2)));
	addType(new CWaypointType(W_FL_TELE_ENTRANCE, "teleentrance", "TF2 engineer bot can build tele entrance here",
	                          WptColor(50, 50, 150), (1 << MOD_TF2)));
	addType(new CWaypointType(W_FL_TELE_EXIT, "teleexit", "TF2 engineer bot can build tele exit here",
	                          WptColor(100, 100, 255), (1 << MOD_TF2)));
	addType(new CWaypointType(W_FL_AREAONLY, "areaonly", "bot will only use this waypoint at certain areas of map",
	                          WptColor(150, 200, 150), (1 << MOD_TF2)));
	addType(new CWaypointType(W_FL_ROUTE, "route", "bot will attempt to go through one of these",
	                          WptColor(100, 100, 100), (1 << MOD_TF2) | (1 << MOD_DOD)));
	addType(new CWaypointType(W_FL_NO_FLAG, "noflag", "TF2 bot will lose flag if he goes thorugh here",
	                          WptColor(200, 100, 50), (1 << MOD_TF2)));
	addType(new CWaypointType(W_FL_COVER_RELOAD, "cover_reload",
	                          "DOD:S bots can take cover here while shooting an enemy and reload. They can also stand "
	                          "up and shoot the enemy after reloading",
	                          WptColor(200, 100, 50), (1 << MOD_DOD)));
	addType(new CWaypointType(W_FL_FLAGONLY, "flagonly", "TF2 bot needs the flag to go through here",
	                          WptColor(180, 50, 80), (1 << MOD_TF2)));
	...
	addType(new CWaypointType(W_FL_MACHINEGUN, "machinegun", "DOD machine gunner will deploy gun here",
	                          WptColor(255, 0, 0), (1 << MOD_DOD)));
	...
	addType(new CWaypointType(W_FL_FALL, "fall", "Bots might kill themselves if they fall down here with low health",
	                          WptColor(128, 128, 128)));
	addType(new CWaypointType(W_FL_CAPPOINT, "capture", "TF2/DOD bot will find a capture point here",
	                          WptColor(255, 255, 0)));
	addType(new CWaypointType(W_FL_BOMBS_HERE, "bombs", "DOD bots can pickup bombs here", WptColor(255, 100, 255),
	                          (1 << MOD_DOD)));
	addType(new CWaypointType(W_FL_BOMB_TO_OPEN, "bombtoopen", "DOD:S bot needs to blow up this point to move on",
	                          WptColor(50, 200, 30), (1 << MOD_DOD)));
	addType(new CWaypointType(W_FL_BREAKABLE, "breakable",
	                          "Bots need to break something with a rocket to get through here", WptColor(100, 255, 50),
	                          (1 << MOD_DOD)));
	addType(new CWaypointType(W_FL_OPENS_LATER, "openslater", "this waypoint is available when a door is open only",
	                          WptColor(100, 100, 200)));
	addType(new CWaypointType(W_FL_WAIT_GROUND, "waitground", "bot will wait until there is ground below",
	                          WptColor(150, 150, 100)));
	addType(new CWaypointType(W_FL_LIFT, "lift", "bot needs to wait on a lift here", WptColor(50, 80, 180)));

	addType(new CWaypointType(W_FL_SPRINT, "sprint", "bots will sprint here", WptColor(255, 255, 190),
	                          ((1 << MOD_DOD) | (1 << MOD_HLDM2))));
	addType(new CWaypointType(W_FL_TELEPORT_CHEAT, "teleport", "bots will teleport to the next waypoint (cheat)",
	                          WptColor(255, 255, 255)));
	addType(new CWaypointType(W_FL_OWNER_ONLY, "owneronly",
	                          "only bot teams who own the area of the waypoint can use it", WptColor(0, 150, 150)));

	// addType(new CWaypointType(W_FL_ATTACKPOINT,"squad_attackpoint","Tactical waypoint -- each squad will go to
	// different attack points and signal others to go",WptColor(90,90,90)));
}
	...
	for (iBot = 0; iBot < 2; iBot++)
	{
		...
		for (i = 0; i < CWaypoints::MAX_WAYPOINTS; i++)
		{
			...
			if (pWpt1->hasSomeFlags(CWaypointTypes::W_FL_RESUPPLY | CWaypointTypes::W_FL_AMMO))
				iCheck =
				    CWaypointTypes::W_FL_SENTRY | CWaypointTypes::W_FL_TELE_ENTRANCE | CWaypointTypes::W_FL_TELE_EXIT;
			...
			if (iCheck != 0)
			{
				for (j = 0; j < CWaypoints::MAX_WAYPOINTS; j++)
				{
					...
					if (pWpt2->hasSomeFlags(iCheck))
					{
						...
						while (pNav->workRoute(pWpt1->getOrigin(), pWpt2->getOrigin(), &bfail, brestart,
						                       bnointerruptions, j)
						       == false)
							;
