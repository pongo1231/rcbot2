...
class CWeapon
{
  public:
	...
	/*CWeapon( int iSlot, const char *szWeaponName, int iId, int iFlags = 0, int iAmmoIndex = -1, float minPrim =0.0f,
	float maxPrim = 4096.0f, int iPref = 0, int iAmmoIndex2 = -1 )
	{
	    m_iSlot = iSlot;
	    setID(iId);
...
class CWeapons
{
  public:
	...
	static inline void addWeapon(CWeapon *pWeapon)
	{
...
class CBotWeapon
{
  public:
	...
	CWeapon *getWeaponInfo()
	{
		return m_pWeaponInfo;
	}

	inline int getWeaponIndex()
	{
		return m_iWeaponIndex;
	}

	inline void setWeaponIndex(int iIndex)
	{
	...
	inline edict_t *getWeaponEntity()
	{
		return m_pEnt;
	}
...
class CBotWeapons
{
  public:
	...
	CBotWeapon *getBestWeapon(edict_t *pEnemy, bool bAllowMelee = true, bool bAllowMeleeFallback = true,
	                          ...
	                          bool bIgnorePrimaryMinimum = false);
	...
	inline void resetSignature()
	{
		m_iWeaponsSignature = 0;
	}
