...
CBotCommandInline PathWaypointOnCommand("on", CMD_ACCESS_WAYPOINT,
...
CBotCommandInline PathWaypointOffCommand("off", CMD_ACCESS_WAYPOINT,
                                         [](CClient *pClient, BotCommandArgs args)
                                         {
...
CBotCommandInline PathWaypointAutoOnCommand("enable", CMD_ACCESS_WAYPOINT,
                                            [](CClient *pClient, BotCommandArgs args)
                                            {
	                                            if (pClient)
...
CBotCommandInline PathWaypointAutoOffCommand("disable", CMD_ACCESS_WAYPOINT,
                                             [](CClient *pClient, BotCommandArgs args)
                                             {
	                                             if (pClient)
...
CBotCommandInline PathWaypointCreate1Command("create1", CMD_ACCESS_WAYPOINT,
                                             [](CClient *pClient, BotCommandArgs args)
                                             {
	                                             if (!pClient)
...
CBotCommandInline PathWaypointCreate2Command("create2", CMD_ACCESS_WAYPOINT,
                                             [](CClient *pClient, BotCommandArgs args)
                                             {
	                                             pClient->updateCurrentWaypoint();
...
CBotCommandInline PathWaypointRemove1Command("remove1", CMD_ACCESS_WAYPOINT,
                                             [](CClient *pClient, BotCommandArgs args)
                                             {
	                                             pClient->updateCurrentWaypoint();
...
CBotCommandInline PathWaypointRemove2Command("remove2", CMD_ACCESS_WAYPOINT,
                                             [](CClient *pClient, BotCommandArgs args)
                                             {
	                                             pClient->updateCurrentWaypoint();
	                                             pClient->setPathTo(pClient->currentWaypoint());
...
CBotCommandInline PathWaypointDeleteToCommand("deleteto", 0,
                                              [](CClient *pClient, BotCommandArgs args)
                                              {
	                                              pClient->updateCurrentWaypoint();
...
CBotCommandInline PathWaypointDeleteFromCommand("deletefrom", 0,
                                                [](CClient *pClient, BotCommandArgs args)
                                                {
	                                                pClient->updateCurrentWaypoint();
...
CBotCommandInline PathWaypointCreateFromToCommand(
    "createfromto", CMD_ACCESS_WAYPOINT,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient && args[0] && *args[0] && args[1] && *args[1])
	    {
		    ...
		    if (pWaypoint && pWaypoint->isUsed())
		    {
			    ...
			    if (pWaypoint2 && pWaypoint2->isUsed())
			    {
				    ...
				    CBotGlobals::botMessage(pClient != NULL ? pClient->getPlayer() : NULL, 0,
				                            "Added path from <%d> to <%d>", atoi(args[0]), atoi(args[1]));
				    ...
				    CBotGlobals::botMessage(pClient != NULL ? pClient->getPlayer() : NULL, 0,
				                            "Waypoint id <%d> not found", atoi(args[1]));
		    }
		    else
			    CBotGlobals::botMessage(pClient != NULL ? pClient->getPlayer() : NULL, 0, "Waypoint id <%d> not found",
			                            atoi(args[0]));
	    }
	    else
		    CBotGlobals::botMessage(pClient != NULL ? pClient->getPlayer() : NULL, 0, "missing args <id1> <id2>");

	    return COMMAND_ERROR;
    });

CBotCommandInline PathWaypointRemoveFromToCommand(
    "removefromto", CMD_ACCESS_WAYPOINT,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient && args[0] && *args[0] && args[1] && *args[1])
	    {
		    ...
		    if (pWaypoint && pWaypoint->isUsed())
		    {
			    ...
			    if (pWaypoint2 && pWaypoint2->isUsed())
			    {
				    ...
				    CBotGlobals::botMessage(pClient != NULL ? pClient->getPlayer() : NULL, 0,
				                            "Removed path from <%d> to <%d>", atoi(args[0]), atoi(args[1]));
