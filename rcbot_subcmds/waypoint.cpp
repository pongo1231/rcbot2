...
CBotCommandInline WaypointOnCommand("on", CMD_ACCESS_WAYPOINT,
...
CBotCommandInline WaypointOffCommand("off", CMD_ACCESS_WAYPOINT,
                                     [](CClient *pClient, BotCommandArgs args)
                                     {
...
CBotCommandInline WaypointAddCommand("add", CMD_ACCESS_WAYPOINT,
                                     [](CClient *pClient, BotCommandArgs args)
                                     {
...
CBotCommandInline WaypointDeleteCommand(
    "delete", CMD_ACCESS_WAYPOINT,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient)
	    {
		    if (args[0] && *args[0])
		    {
			    ...
			    if (radius > 0)
			    {
				    ...
				    if (numdeleted > 0)
				    {
					    CBotGlobals::botMessage(pClient->getPlayer(), 0, "%d waypoints within range of %0.0f deleted",
					                            numdeleted, radius);
...
CBotCommandInline WaypointInfoCommand("info", 0,
                                      [](CClient *pClient, BotCommandArgs args)
                                      {
	                                      pClient->updateCurrentWaypoint();
...
CBotCommandInline WaypointSaveCommand("save", CMD_ACCESS_WAYPOINT,
                                      [](CClient *pClient, BotCommandArgs args)
                                      {
	                                      if (CWaypoints::save(
	                                              false, (pClient != nullptr) ? pClient->getPlayer() : nullptr,
	                                              ((args[0] != nullptr) && (*args[0] != 0)) ? args[0] : nullptr,
	                                              ((args[1] != nullptr) && (*args[1] != 0)) ? args[1] : nullptr))
	                                      {
...
CBotCommandInline WaypointLoadCommand("load", CMD_ACCESS_WAYPOINT,
                                      [](CClient *pClient, BotCommandArgs args)
                                      {
	                                      ...
	                                      if (bLoadOK)
		                                      ...
		                                      CBotGlobals::botMessage(nullptr, 0, "error: could not load %s waypoints",
		                                                              szMapName);

	                                      return COMMAND_ACCESSED;
                                      });

CBotCommandInline WaypointClearCommand("clear", CMD_ACCESS_WAYPOINT,
                                       [](CClient *pClient, BotCommandArgs args)
                                       {
	                                       CWaypoints::init();
...
CBotCommandInline WaypointGiveTypeCommand(
    "givetype", CMD_ACCESS_WAYPOINT,
    [](CClient *pClient, BotCommandArgs args)
    {
	    ...
	    if (args[0] && *args[0])
	    {
		    if (pClient->currentWaypoint() == -1)
			    CBotGlobals::botMessage(
			        pEntity, 0,
			        "No waypoint nearby to give types (move closer to the waypoint you want to give types)");
		    else
		    {
			    char *type = nullptr;
				    ...
				    if (pType)
				    {
					    ...
					    if (pWaypoint)
					    {
						    if (pWaypoint->hasFlag(pType->getBits()))
						    {
							    ...
							    CBotGlobals::botMessage(pEntity, 0, "type %s removed from waypoint %d", type,
							                            CWaypoints::getWaypointIndex(pWaypoint));
						    ...
						    else
						    {
							    ...
							    CBotGlobals::botMessage(pEntity, 0, "type %s added to waypoint %d", type,
							                            CWaypoints::getWaypointIndex(pWaypoint));
...
CBotCommandInline WaypointDrawTypeCommand(
    "drawtype", CMD_ACCESS_WAYPOINT,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient)
	    {
    ...
    "0: for effects engine (maximum limit of beams)\n1: for debug overlay (no limit of beams) [LISTEN SERVER CLIENT "
    "ONLY]");

CBotCommandInline WaypointAngleCommand(
    "angle", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient && pClient->getPlayer())
	    {
		    ...
		    if (pWpt)
		    {
			    ...
			    CBotGlobals::botMessage(pClient->getPlayer(), 0, "Waypoint Angle == %0.3f deg, (Eye == %0.3f)",
			                            CBotGlobals::yawAngleFromEdict(pClient->getPlayer(), pWpt->getOrigin()), eye.y);
...
CBotCommandInline WaypointSetAngleCommand("updateyaw", 0,
                                          [](CClient *pClient, BotCommandArgs args)
                                          {
	                                          pClient->updateCurrentWaypoint();
...
CBotCommandInline WaypointSetAreaCommand(
    "setarea", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
	    pClient->updateCurrentWaypoint();
...
CBotCommandInline WaypointSetRadiusCommand(
    "setradius", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
	    pClient->updateCurrentWaypoint();
...
CBotCommandInline WaypointMenuCommand("menu", 0,
                                      [](CClient *pClient, BotCommandArgs args)
                                      {
	                                      ...
	                                      if ( pClient )
	                                      {
	                                          ...
	                                          if ( !pWpt )
	                                          {
	                                              CBotGlobals::botMessage(pClient->getPlayer(),0,"Not next to any
	                                      waypoints to show a menu for"); return COMMAND_ERROR;
	                                          }
	                                          ...
	                                          while ( (i < iNumToShow) && (iIndex < iNumTypes) )
	                                          {
	                                              ...
	                                              Q_snprintf( cmd, sizeof(cmd), "rcbot waypoint givetype %s",
	                                      p->getName() );
	                                          ...
	                                          helpers->CreateMessage( pClient->getPlayer(), DIALOG_MENU, kv,
	                                      &g_RCBOTServerPlugin ); kv->deleteThis();
...
CBotCommandInline WaypointCopy(
    "copy", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient)
    ...
    "Go to a waypoint, and copy to hold its properties, then use paste to make a new waypoint with the same "
    "properties");

CBotCommandInline WaypointPaste(
    "paste", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient)
...
CBotCommandInline WaypointShiftAreas(
    "shiftareas", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient)
...
CBotCommandInline WaypointTeleportCommand("teleport", 0,
                                          [](CClient *pClient, BotCommandArgs args)
                                          {
...
CBotCommandInline WaypointAreaSetToNearest(
    "setareatonearest", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient)
...
CBotCommandInline WaypointShowCommand(
    "show", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
	    if (pClient)
	    {
		    ...
		    if (pwpt)
		    {
			    g_pEffects->Beam(CBotGlobals::entityOrigin(pClient->getPlayer()), pwpt->getOrigin(),
			                     CWaypoints::waypointTexture(), 0, 0, 1, 5, 12, 12, 255, 10, 255, 255, 255, 200, 10);
...
CBotCommandInline WaypointCheckCommand("check", 0,
                                       [](CClient *pClient, BotCommandArgs args)
                                       {
	                                       // loop through every waypoint and check the areas are not outside the number
	                                       // of control points

	                                       CWaypoints::checkAreas((pClient == nullptr) ? nullptr
	                                                                                   : pClient->getPlayer());

	                                       return COMMAND_ACCESSED;
                                       });

CBotCommandInline WaypointShowVisCommand(
    "showvis", 0,
    [](CClient *pClient, BotCommandArgs args)
    {
#ifndef __linux__
	    ...
	    if (CWaypoints::validWaypointIndex(pClient->currentWaypoint()))
	    {
		    ...
		    if (pWpt)
		    {
			    ...
			    for (i = 0; i < CWaypoints::numWaypoints(); i++)
			    {
				    ...
				    debugoverlay->AddTextOverlayRGB(pOther->getOrigin(), 0, ftime, bVis ? 0 : 255, bVis ? 255 : 0, 0,
				                                    200, bVis ? "VIS" : "INV");
			    }
		    }
	    }
...
CBotCommandInline WaypointAutoWaypointCommand("autowaypoint", CMD_ACCESS_WAYPOINT,
                                              [](CClient *pClient, BotCommandArgs args)
                                              {
	                                              if (pClient)
	                                              {
		                                              pClient->setAutoWaypointMode(atoi(args[0]) > 0,
		                                                                           atoi(args[0]) == 2);
		                                              CBotGlobals::botMessage(pClient->getPlayer(), 0,
		                                                                      "Autowaypointing Mode %s, Debug %s",
		                                                                      (atoi(args[0]) > 0) ? "ON" : "OFF",
		                                                                      (atoi(args[0]) == 2) ? "ON" : "OFF");
	                                              }
	                                              return COMMAND_ACCESSED;
                                              });

CBotCommandInline WaypointAutoFix("autofix", 0,
                                  [](CClient *pClient, BotCommandArgs args)
                                  {
