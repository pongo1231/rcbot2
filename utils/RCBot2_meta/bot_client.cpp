		...
		if (isDebugOn(BOT_DEBUG_USERCMD))
		{
			...
			if (p)
			{
				...
				CBotGlobals::botMessage(
				    m_pPlayer, 0, "Btns = %d, cmd_no = %d, impulse = %d, weapselect = %d, weapsub = %d", cmd.buttons,
				    cmd.command_number, cmd.impulse, cmd.weaponselect, cmd.weaponsubtype);
			}
		}
		...
		else
		{
							...
							CWaypoints::addWaypoint(m_pPlayer, m_vAutoEventWaypointOrigin,
							                        (m_iAutoEventWaypointTeam == 0)
							                            ...
							                            : ((m_iAutoEventWaypoint | m_iAutoEventWaypointTeamOn)
							                               & ~m_iAutoEventWaypointTeamOff),
							                        true, cmd.viewangles.y, 0, 32.0f);
					}
					else
						CWaypoints::addWaypoint(
						    m_pPlayer, m_vAutoEventWaypointOrigin,
						    (m_iAutoEventWaypointTeam == 0)
						        ? (m_iAutoEventWaypoint)
						        : ((m_iAutoEventWaypoint | m_iAutoEventWaypointTeamOn) & ~m_iAutoEventWaypointTeamOff),
						    true, cmd.viewangles.y, 0, 32.0f);
					//}
					/*else
					{
			...
			if (/*(pev->waterlevel < 3) &&*/ (m_fCanPlaceJump < engine->Time()))
			{
				...
				if ((m_fCanPlaceJump != -1) && (m_iLastButtons & IN_JUMP) && !(iPlayerFlags & FL_ONGROUND))
				{
					...
					if (iNearestWpt == -1)
					{
						m_iLastJumpWaypointIndex =
						    CWaypoints::addWaypoint(m_pPlayer, vPlayerOrigin, CWaypointTypes::W_FL_JUMP, true);
					}
					else
			...
			bool bCheckDistance =
			    (iMoveType != MOVETYPE_FLY) && (m_fCanPlaceLadder == 0); // always check distance unless ladder climbing
			...
			if ((iMoveType == MOVETYPE_FLY) && !(m_iLastMoveType == MOVETYPE_FLY))
			{
				...
				if (iNearestWpt == -1)
					m_iLastLadderWaypointIndex =
					    CWaypoints::addWaypoint(m_pPlayer, vPlayerOrigin, CWaypointTypes::W_FL_LADDER, true);
				else
				{
			...
			if (m_fLastAutoWaypointCheckTime < engine->Time())
			{
				...
				if ((m_iLastJumpWaypointIndex == -1) && bCheckDistance
				...
				for (i = 0; i < MAX_STORED_AUTOWAYPOINT; i++)
				{
					...
					{
						debugoverlay->AddLineOverlay(vCheckOrigin + Vector(0, 0, 16), vCheckOrigin - Vector(0, 0, 16),
						                             255, 255, 255, 0, 2);
