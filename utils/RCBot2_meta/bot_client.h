...
template <class T> class CTypeVector
{
  public:
	CTypeVector()
	{
...
class CClient
{
  public:
	...
	inline CBotMenu *getCurrentMenu()
	{
		return m_pMenu;
	}
	inline void setMenuCommand(int iCommand)
	{
		m_iMenuCommand = iCommand;
	}
	inline int getLastMenuCommand()
	{
		return m_iMenuCommand;
	}
	...
	inline bool isPlayer(edict_t *pPlayer)
	{
		return m_pPlayer == pPlayer;
	}

	inline bool isWaypointOn()
	{
		return m_bWaypointOn;
	}
	inline void setWaypointOn(bool bOn)
	{
		m_bWaypointOn = bOn;
	}
	inline void setWaypoint(int iWpt)
	{
		m_iCurrentWaypoint = iWpt;
	}
	inline int currentWaypoint()
	{
		return m_iCurrentWaypoint;
	}

	inline void setAccessLevel(int iLev)
	{
		m_iAccessLevel = iLev;
	}

	inline bool isAutoPathOn()
	{
		return m_bAutoPaths;
	}
	inline void setAutoPath(bool bOn)
	{
		m_bAutoPaths = bOn;
	}
	inline bool isPathWaypointOn()
	{
		return m_bPathWaypointOn;
	}
	inline void setPathWaypoint(bool bOn)
	{
		m_bPathWaypointOn = bOn;
	}

	inline int getWptArea()
	{
		return m_iWptArea;
	}
	inline void setWptArea(int area)
	{
		m_iWptArea = area;
	}

	inline void setPathFrom(int iWpt)
	{
		m_iPathFrom = iWpt;
	}
	inline void setPathTo(int iWpt)
	{
		m_iPathTo = iWpt;
	}

	inline int getPathFrom()
	{
		return m_iPathFrom;
	}
	inline int getPathTo()
	{
		return m_iPathTo;
	}
	...
	{
		return (m_iDebugLevels & (1 << iLevel)) > 0;
	}
	void clearDebug()
	{
		m_iDebugLevels = 0;
	...
	bool isDebugging()
	{
		return (m_iDebugLevels != 0);
	}

	inline void setDebugBot(edict_t *pBot)
	{
		m_pDebugBot = pBot;
	}
	inline bool isDebuggingBot(edict_t *pBot)
	{
		return m_pDebugBot == pBot;
	}
	inline edict_t *getDebugBot()
	{
		return m_pDebugBot;
	}
	...
	inline void setDrawType(unsigned short int iType)
	{
		m_iWaypointDrawType = iType;
	}
	inline unsigned short int getDrawType()
	{
		return m_iWaypointDrawType;
	}

	inline float getWptCopyRadius()
	{
		return m_fCopyWptRadius;
	}
	inline int getWptCopyFlags()
	{
		return m_iCopyWptFlags;
	}
	inline int getWptCopyArea()
	{
		return m_iCopyWptArea;
	}

	inline eWptCopyType getWptCopyType()
	{
		return m_WaypointCopyType;
	}

	inline bool isShowingWaypoint(int iFlags)
	{
		return (m_iWaypointShowFlags & iFlags) > 0;
	}
	inline void showWaypoints(int iFlags)
	{
		m_iWaypointShowFlags |= iFlags;
	}
	inline void dontShowWaypoints(int iFlags)
	{
		m_iWaypointShowFlags &= ~iFlags;
	}
	inline bool isShowingAllWaypoints()
	{
		return m_iWaypointShowFlags == 0;
	}
	inline int getShowWaypointFlags()
	{
		return m_iWaypointShowFlags;
	}
	...
	inline bool autoWaypointOn()
	{
		return m_bAutoWaypoint;
	}
	void autoEventWaypoint(int iType, float fRadius, bool bAtOtherOrigin = false, int iTeam = 0,
	                       Vector vOrigin = Vector(0, 0, 0), bool bIgnoreTeam = false, bool bAutoType = false);
...
class CClients
{
  public:
	...
	static CClient *get(int iIndex)
	{
		return &m_Clients[iIndex];
	}
	static CClient *get(edict_t *pPlayer)
	{
		return &m_Clients[slotOfEdict(pPlayer)];
	}
	static void setListenServerClient(CClient *pClient)
	{
		m_pListenServerClient = pClient;
	}
	static bool isListenServerClient(CClient *pClient)
	{
		return m_pListenServerClient == pClient;
	}
