...
#define NULLIFY_VISIBLE(visobj, pent, distance)           \
	if (visobj == pent)                                   \
	{                                                     \
		if (!bValid || (distanceFrom(visobj) > distance)) \
		{                                                 \
			visobj = NULL;                                \
		}                                                 \
	}
#define NULLIFY_VISIBLE_CONDITION(visobj, pent, distance, condition)     \
	if (visobj == pent)                                                  \
	{                                                                    \
		if (!bValid || (distanceFrom(visobj) > distance) || (condition)) \
		{                                                                \
			visobj = NULL;                                               \
		}                                                                \
	}
...
bool CDODBot ::setVisible(edict_t *pEntity, bool bVisible)
{
	...
	{
		if ((m_pNearestDeadTeamMate != pEntity) && (CClassInterface::getTeam(pEntity) == m_iTeam)
		    && !CBotGlobals::entityIsAlive(pEntity))
		{
		...
		{
			UPDATE_VISIBLE_OBJECT_CONDITION(m_pNearestBreakable, pEntity,
		...
		else if ((pEntity != m_pNearestPathBomb) && CDODMod::isPathBomb(pEntity)
		         && (CClassInterface::getDODBombState(pEntity) != 0))
		{
	...
	else
	{
		...
		NULLIFY_VISIBLE_CONDITION(m_pNearestBreakable, pEntity, CWaypointLocations::REACHABLE_RANGE,
		                          CClassInterface::getPlayerHealth(m_pNearestBreakable) <= 0);
		NULLIFY_VISIBLE(m_pEnemyGrenade, pEntity, BLAST_RADIUS * 2);
		NULLIFY_VISIBLE(m_pEnemyRocket, pEntity, BLAST_RADIUS * 2);
		NULLIFY_VISIBLE_CONDITION(m_pNearestBomb, pEntity, BLAST_RADIUS * 2,
		                          CClassInterface::getDODBombState(m_pNearestBomb) == 0);
		NULLIFY_VISIBLE_CONDITION(m_pNearestPathBomb, pEntity, BLAST_RADIUS * 2,
		...
		if ((fSmokeTime >= 1.0f) && (fSmokeTime <= 10.0f))
		{
			...
			else if (m_pEnemy && hasSomeConditions(CONDITION_SEE_CUR_ENEMY))
			{
				    ...
				    || (distanceFrom(pEntity) < (distanceFrom(m_pEnemy) + SMOKE_RADIUS)))
				{
					...
					if (m_pNearestSmokeToEnemy
					    && (fabs(DotProductFromOrigin(CBotGlobals::entityOrigin(pEntity))
...
bool CDODBot ::startGame()
{
	...
	if ((m_iDesiredClass >= 0) && (m_iDesiredClass <= 5)
	    && (m_iDesiredClass != CClassInterface::getPlayerClassDOD(m_pEdict)))
	{
		kill();
		changeClass();
...
void CDODBot ::seeFriendlyDie(edict_t *pDied, edict_t *pKiller, CWeapon *pWeapon)
{
	...
	if ((pKiller != m_pEdict) && pKiller && !m_pEnemy && !hasSomeConditions(CONDITION_SEE_CUR_ENEMY)
	    && isEnemy(pKiller, false))
	{
		...
		if (pWeapon)
		{
			...
			{
				if ((m_LastHearVoiceCommand == DOD_VC_SNIPER) && m_pWeapons->hasWeapon(DOD_WEAPON_FRAG_US)
				    && !m_pWeapons->hasWeapon(DOD_WEAPON_FRAG_GER))
				...
				ADD_UTILITY_DATA_VECTOR(BOT_UTIL_SNIPE_POINT,
				                        !hasEnemy() && (m_iClass == DOD_CLASS_SNIPER) && getSniperRifle()
				                            && !getSniperRifle()->outOfAmmo(this),
				                        1.0f, (unsigned int)pKiller, vecEnemy);
			}
			else if ((pclass == DOD_CLASS_MACHINEGUNNER) && pWeapon->isDeployable())
			{
				if ((m_LastHearVoiceCommand == DOD_VC_MGAHEAD) && m_pWeapons->hasWeapon(DOD_WEAPON_FRAG_US)
				    && !m_pWeapons->hasWeapon(DOD_WEAPON_FRAG_GER))
					addVoiceCommand(DOD_VC_USE_GRENADE);
				else
				...
				updateCondition(CONDITION_COVERT);
				m_pNavigator->belief(CBotGlobals::entityOrigin(pDied), CBotGlobals::entityOrigin(pKiller), 100.0f,
				                     512.0f, BELIEF_DANGER);
				...
				ADD_UTILITY_DATA_VECTOR(BOT_UTIL_SNIPE_POINT,
				                        !hasEnemy() && (m_iClass == DOD_CLASS_SNIPER) && getSniperRifle()
				                            && !getSniperRifle()->outOfAmmo(this),
				                        1.0f, (unsigned int)pKiller, vecEnemy);
				// ADD_UTILITY_DATA_VECTOR(BOT_UTIL_MOVEUP_MG,!hasEnemy() && (m_iClass == DOD_CLASS_MACHINEGUNNER) &&
				// getMG() && !getMG()->outOfAmmo(this),1.0f,1,vecEnemy);
			}
			else
			{
			...
			if (isVisible(pKiller))
			{
				...
				int iEnemyWaypoint =
				    CWaypointLocations::NearestWaypoint(CBotGlobals::entityOrigin(pKiller), 100.0f, -1, true, true);

				if ((iCurrentWaypoint != -1) && (iEnemyWaypoint != -1)
				    && !pTable->GetVisibilityFromTo(iCurrentWaypoint, iEnemyWaypoint))
				{
					// bFollow = false;

					ADD_UTILITY_DATA_VECTOR(BOT_UTIL_COVER_POINT, m_pCurrentWeapon != NULL, 0.8f,
					                        ((unsigned int)pKiller), (vecEnemy));
				}
			}
			/*else if ( CBotGlobals::isPlayer(pDied) )
			{
			    ...
			    {
			        CWaypoint *pWpt =
			CWaypoints::getPinchPointFromWaypoint(getOrigin(),CWaypoints::getWaypoint(iWpt)->getOrigin());
		...
		pWpt                         = CWaypoints::getWaypoint(CWaypointLocations::NearestBlastWaypoint(
		...
		if (!hasEnemy())
		{
			...
			// move up MG
			if (!rcbot_melee_only.GetBool() && pMachineGun && (m_iClass == DOD_CLASS_MACHINEGUNNER)
			    && !pMachineGun->outOfAmmo(this) && pMachineGun->isDeployable())
			{
				CWaypoint *pWaypoint;
				...
				if (!m_pSchedules->hasSchedule(SCHED_DEPLOY_MACHINE_GUN))
				{
					...
					vSearchForMachineGunPointOrigin =
					    vSearchForMachineGunPointOrigin / vSearchForMachineGunPointOrigin.Length();

					vSearchForMachineGunPointOrigin =
					    getOrigin() + (vSearchForMachineGunPointOrigin * CWaypointLocations::REACHABLE_RANGE);

					if ((pWaypoint = CWaypoints::getWaypoint(CWaypointLocations::NearestWaypoint(
					         vSearchForMachineGunPointOrigin, CWaypointLocations::REACHABLE_RANGE, -1, false, false,
					         false, NULL, false, m_iTeam, false, false, Vector(0, 0, 0),
					         CWaypointTypes::W_FL_MACHINEGUN)))
					    != NULL)
					{
						ADD_UTILITY_WEAPON_DATA_VECTOR(BOT_UTIL_MOVEUP_MG, pMachineGun && !pMachineGun->outOfAmmo(this),
						                               1.0f, pMachineGun, CWaypoints::getWaypointIndex(pWaypoint),
						                               m_vListenPosition);
			...
			if (!m_pSchedules->isCurrentSchedule(SCHED_INVESTIGATE_NOISE))
			{
				ADD_UTILITY_DATA_VECTOR(BOT_UTIL_INVESTIGATE_POINT,
				                        !m_pSchedules->hasSchedule(SCHED_DEPLOY_MACHINE_GUN)
				                            && !m_pSchedules->hasSchedule(SCHED_SNIPE),
				                        0.5f, (unsigned int)pDied, m_vListenPosition);

				// m_pSchedules->removeSchedule(SCHED_INVESTIGATE_NOISE);
				// m_pSchedules->addFront(new
				// CBotInvestigateNoiseSched(CBotGlobals::entityOrigin(pDied),m_vListenPosition));
			}
...
void CDODBot ::seeFriendlyKill(edict_t *pTeamMate, edict_t *pDied, CWeapon *pWeapon)
{
	...
	if ((pDied != m_pEdict) && pTeamMate && !hasSomeConditions(CONDITION_SEE_CUR_ENEMY)
	    && (CClassInterface::getTeam(pDied) != m_iTeam))
	{
		m_fLastSeeEnemy = engine->Time();
		...
		if (pWeapon)
		{
			...
			if ((pclass == DOD_CLASS_MACHINEGUNNER) && pWeapon->isDeployable())
			{
				...
				m_pNavigator->belief(CBotGlobals::entityOrigin(pTeamMate), CBotGlobals::entityOrigin(pDied), MAX_BELIEF,
				                     512.0f, BELIEF_SAFETY);
				updateCondition(CONDITION_CHANGED);
...
void CDODBot ::handleWeapons()
{
	...
	if (m_pEnemy && !hasSomeConditions(CONDITION_ENEMY_DEAD) && hasSomeConditions(CONDITION_SEE_CUR_ENEMY)
	    && wantToShoot() && isVisible(m_pEnemy) && isEnemy(m_pEnemy, true))
	{
		CBotWeapon *pWeapon;

		pWeapon = getCurrentWeapon();

		if (pWeapon && pWeapon->getWeaponEntity() && !rcbot_melee_only.GetBool() && pWeapon->isDeployable()
		    && !pWeapon->outOfAmmo(this) && CClassInterface::isMachineGunDeployed(pWeapon->getWeaponEntity()))
		{
			; // keep current weapon on
		}
		else
			pWeapon = getBestWeapon(
			    m_pEnemy, true, true, rcbot_melee_only.GetBool(),
			    !rcbot_melee_only.GetBool()
			        && (((m_pEnemy == m_pNearestBreakable) || CDODMod::isBreakableRegistered(m_pEnemy, m_iTeam))));
...
void CDODBot ::touchedWpt(CWaypoint *pWaypoint, int iNextWaypoint, int iPrevWaypoint)
{
		...
		if (CBotGlobals::entityIsValid(pBombTarget))
		{
			...
			if (pBombTarget && (state != 0) && (CClassInterface::getDODBombTeam(pBombTarget) == m_iTeam))
			{
				...
				// plant
				{
					...
					bombsched->addTask(new CBotDODBomb(DOD_BOMB_PATH_PLANT, -1, pBombTarget,
					                                   CBotGlobals::entityOrigin(pBombTarget), -1));
					bombsched->addTask(new CDODWaitForBombTask(pBombTarget, pWaypoint));
	...
	else
	{
		...
		if ((iPrevWaypoint != -1) && (iNextWaypoint != -1)
		    && (randomFloat(0, MAX_BELIEF) < m_pNavigator->getBelief(CWaypoints::getWaypointIndex(pWaypoint))))
		{
			...
			for (i = 0; i < pWaypoint->numPaths(); i++)
			{
				...
				if ((pTable->GetVisibilityFromTo(iPrevWaypoint, iPath) == false)
				    && (pTable->GetVisibilityFromTo(iThisWaypoint, iPath) == true)
				    && (pTable->GetVisibilityFromTo(iNextWaypoint, iPath) == false))
				{
					...
					{
						debugoverlay->AddLineOverlay(CWaypoints::getWaypoint(iNextWaypoint)->getOrigin(),
						                             pPath->getOrigin(), 255, 120, 120, false, 7.0f);
						debugoverlay->AddLineOverlay(pWaypoint->getOrigin(), pPath->getOrigin(), 255, 255, 255, false,
						                             7.0f);
					}

#endif
					//					debugoverlay->AddTextOverlayRGB((pWaypoint->getOrigin()+pPath->getOrigin())/2,0,7.0f,255,255,255,255,"Dot:
					//%0.3f",flDot);
				}
			}
...
void CDODBot ::modThink()
{
	...
	if (!isUnderWater() && !hasSomeConditions(CONDITION_RUN) && (m_fCurrentDanger >= 50.0f))
	{
		...
		if (hasSomeConditions(CONDITION_COVERT) && m_pCurrentWeapon && pWeapon
		    && ((pWeapon->getID() == DOD_WEAPON_K98) || (pWeapon->getID() == DOD_WEAPON_GARAND)))
		{
			bool bZoomed = false;
		...
		if ((hasSomeConditions(CONDITION_PRONE) || !rcbot_prone_enemy_only.GetBool()
		     || ((m_pEnemy.get() != NULL) || (m_fLastSeeEnemy + 5.0f > engine->Time())))
	...
	else if (hasSomeConditions(CONDITION_RUN)
	         || ((m_fCurrentDanger >= 20.0f) && (m_flStamina >= 5.0f) && (m_flSprintTime < engine->Time())))
	{
		// unprone
	...
	if ((pWeapon && pWeapon->needToReload(this)) || (m_fLastSeeEnemy && ((m_fLastSeeEnemy + 5.0) < engine->Time())))
	{
		m_fLastSeeEnemy = 0;
	...
	if (hasSomeConditions(CONDITION_NEED_AMMO) && (m_fNextCheckNeedAmmo < engine->Time())
	    && (m_StatsCanUse.stats.m_iTeamMatesVisible > 0) && (m_StatsCanUse.stats.m_iTeamMatesInRange > 0))
	{
	...
	if (m_pNearestPathBomb)
	{
		...
		if (CClassInterface::getDODBombState(m_pNearestPathBomb) == 2)
		{
			...
			{
				// run
				if (!m_pSchedules->hasSchedule(SCHED_BOMB) && !m_pSchedules->hasSchedule(SCHED_GOOD_HIDE_SPOT))
				{
					...
					if (pWaypoint)
					{
						CBotSchedule *runsched =
						    new CBotSchedule(new CDODWaitForBombTask(m_pNearestPathBomb, pWaypoint));
	...
	if (m_pNearestBomb)
	{
		...
		{
			if (!CDODMod::m_Flags.isTeamMateDefusing(m_pEdict, m_iTeam, iBombID)
			    && !m_pSchedules->hasSchedule(SCHED_BOMB))
			{
				...
				attack->addTask(new CBotDODBomb(DOD_BOMB_DEFUSE, iBombID, m_pNearestBomb,
				                                CBotGlobals::entityOrigin(m_pNearestBomb), -1));
				// add defend task
				m_pSchedules->freeMemory();
		...
		else if (!m_pSchedules->hasSchedule(SCHED_BOMB) && !m_pSchedules->hasSchedule(SCHED_GOOD_HIDE_SPOT)
		         && !CDODMod::m_Flags.isBombBeingDefused(iBombID) && CDODMod::m_Flags.isBombPlanted(iBombID)
...
void CDODBot ::hearVoiceCommand(edict_t *pPlayer, byte cmd)
{
	...
	case DOD_VC_NEED_MG:
		...
		IF_WANT_TO_LISTEN
		{
			if (hasMG())
			{
				...
				{
					CWaypoint *pWaypoint =
					    CWaypoints::randomWaypointGoal(CWaypointTypes::W_FL_MACHINEGUN, m_iTeam, iFlagID, true, this);
		...
		else if (randomFloat(0.0f, 1.0f) < 0.2f) addVoiceCommand(DOD_VC_NO);
		break;
		...
		IF_WANT_TO_LISTEN
		{
			...
			if (p)
			{
				...
				if (cmd == DOD_VC_GO_LEFT)
					vOrigin = vOrigin - (vRight * (CWaypointLocations::BUCKET_SPACING + 1))
					        + (vForward * (CWaypointLocations::BUCKET_SPACING + 1));
				else
					vOrigin = vOrigin + (vRight * (CWaypointLocations::BUCKET_SPACING + 1))
					        + (vForward * (CWaypointLocations::BUCKET_SPACING + 1));
				...
				CWaypoint *pWaypoint = CWaypoints::getWaypoint(
				    CWaypointLocations::NearestWaypoint(vOrigin, 768.0f, -1, false, false, false, NULL, false, 0, false,
				                                        false, Vector(0, 0, 0), CWaypointTypes::W_FL_ROUTE, NULL));
				...
				else
				{
					...
					CWaypoint *pWaypoint = CWaypoints::getWaypoint(CWaypointLocations::NearestWaypoint(
					    vOrigin, 768.0f, -1, false, false, false, NULL, false, 0, false, false, Vector(0, 0, 0),
					    CWaypointTypes::W_FL_CAPPOINT, NULL));
	...
	case DOD_VC_GOGOGO:
		...
		else if (randomFloat(0.0f, 1.0f) > 0.9f) addVoiceCommand(DOD_VC_NO);
		break;
	case DOD_VC_HOLD:
		if (inSquad() && (m_pSquad->GetLeader() == pPlayer))
		{
	...
	case DOD_VC_NEED_AMMO:
		...
		if (!m_bDroppedAmmoThisRound)
		{
			...
			if ((!inSquad() && (distanceFrom(pPlayer) < 512.0f) && isVisible(pPlayer))
			    || (inSquad() && m_pSquad->IsMember(pPlayer)))
			{
				if (!m_pSchedules->isCurrentSchedule(SCHED_DOD_DROPAMMO))
				{
					...
					if (!isVisible(pPlayer) || (distanceFrom(pPlayer) > 200.0f))
					{
						CWaypoint *pWpt = CWaypoints::getWaypoint(
						    CWaypointLocations::NearestWaypoint(CBotGlobals::entityOrigin(pPlayer), 200.0f, -1));
		...
		else if (!inSquad() && isVisible(pPlayer))
		{
			...
			else
			{
				if (randomFloat(0.0f, 1.0f) > 0.25f)
		         ...
		         || ((randomFloat(0.0f, 1.0f) > 0.75f) && rcbot_bots_form_squads.GetBool())) // already in a squad
		{
	...
	case DOD_VC_BAZOOKA:
		...
		if (!m_pEnemyGrenade && !m_pEnemyRocket
		    && ((inSquad() && (m_pSquad->GetLeader() == pPlayer)) || isVisible(pPlayer))
	...
	case DOD_VC_NEED_BACKUP:
		if ((!inSquad() || (m_pSquad->GetLeader() == m_pEdict)) && m_pNearestFlag && isVisible(pPlayer)
		    && !rcbot_nocapturing.GetBool())
		{
			...
			if ((vPoint - vPlayer).Length() < 250)
			{
				...
				if (CDODMod::isBombMap())
				{
					CWaypoint *pWpt = CWaypoints::getWaypoint(CWaypointLocations::NearestBlastWaypoint(
					    vPoint, vPlayer, 1000.0, -1, true, false, true, false, m_iTeam, true));
...
bool CDODBot ::executeAction(CBotUtility *util)
{
	...
	case BOT_UTIL_INVESTIGATE_POINT:
		m_pSchedules->removeSchedule(SCHED_INVESTIGATE_NOISE);
		m_pSchedules->addFront(new CBotInvestigateNoiseSched(CBotGlobals::entityOrigin((edict_t *)util->getIntData()),
		                                                     util->getVectorData()));
		return true;
	...
	case BOT_UTIL_SNIPE_POINT:
		// find sniper point facing the enemy
		{
			...
			if (pNearest != NULL)
			{
				...
				snipesched->addTask(new CBotDODSnipe(getSniperRifle(), pNearest->getOrigin(), pNearest->getAimYaw(),
				                                     true, vEnemyOrigin.z, pNearest->getFlags()));
	...
	case BOT_UTIL_MOVEUP_MG:
		m_pSchedules->add(new CDeployMachineGunSched(
	...
	case BOT_UTIL_DEFEND_NEAREST_POINT:
	{
		...
		if (pWaypoint)
		{
			...
			CBotTask *deftask    = new CBotDefendTask(pWaypoint->getOrigin(), randomFloat(7.5f, 12.5f), 0, true, vGoal,
                                                   defend_wpt ? LOOK_SNIPE : LOOK_AROUND, pWaypoint->getFlags());
	...
	case BOT_UTIL_SNIPE: // find snipe or machine gun waypoint
	{
		...
		if (inSquad() && !isSquadLeader())
		{
			...
			if (iFlagID == -1)
			{
				pWaypoint = CWaypoints::getWaypoint(CWaypointLocations::NearestWaypoint(
				    m_pSquad->GetFormationVector(m_pEdict), 250.0f, -1, false, false, true, NULL, false, m_iTeam, false,
				    false, Vector(0, 0, 0), iWaypointType));
			}
			else
		...
		else
		{
			if (util->getIntData()) // attack
			{
				...
				else
				{
					...
					if (randomFloat(0.0f, 1.0f) < ((float)CDODMod::m_Flags.getNumPlantableBombs(m_iTeam)
					                               / CDODMod::m_Flags.getNumBombsOnMap(m_iTeam)))
					{
						pWaypoint = CWaypoints::randomWaypointGoal(iWaypointType, m_iTeam,
						                                           CDODMod::getBombPointArea(m_iTeam), true, this);
					}
					else
					{
						// attack a point
		...
		if (pWaypoint)
		{
			...
			{
				CWaypoint *pBombs = CWaypoints::randomWaypointGoal(CWaypointTypes::W_FL_BOMBS_HERE, m_iTeam,
				                                                   pWaypoint->getArea(), true);
			...
			snipetask = new CBotDODSnipe(util->getWeaponChoice(), pWaypoint->getOrigin(), pWaypoint->getAimYaw(),
			                             iFlagID != -1, vGoal.z + 48, pWaypoint->getFlags());
	...
	case BOT_UTIL_DEFEND_POINT:
	{
		...
		if (pWaypoint)
		{
			...
			CBotTask *deftask    = new CBotDefendTask(pWaypoint->getOrigin(), randomFloat(7.5f, 12.5f), 0, false,
			                                          Vector(0, 0, 0), LOOK_SNIPE, pWaypoint->getFlags());
	...
	case BOT_UTIL_PLANT_BOMB:
	{
		...
		if (pWaypoint && (iBombType == DOD_BOMB_PLANT)
		    && (randomFloat(0.0f, 200.0f) < m_pNavigator->getBelief(CWaypoints::getWaypointIndex(pWaypoint))))
		{
			...
			attack->addTask(new CBotInvestigateTask(pWaypoint->getOrigin(), 250, Vector(0, 0, 0), false,
			                                        randomFloat(3.0f, 5.0f), CONDITION_SEE_CUR_ENEMY));
		}
	...
	case BOT_UTIL_PICKUP_BOMB:
	{
		...
		pWaypoint      = CWaypoints::getWaypoint(
            CWaypoints::nearestWaypointGoal(CWaypointTypes::W_FL_BOMBS_HERE, vOrigin, 8192.0, m_iTeam));

		if (pWaypoint)
		{
			if (CDODMod::isCommunalBombPoint())
			{
				CWaypoint *pWaypointPinch = CWaypoints::randomWaypointGoal(
				    CWaypointTypes::W_FL_DEFEND | CWaypointTypes::W_FL_SNIPER | CWaypointTypes::W_FL_MACHINEGUN,
				    m_iTeam, pWaypoint->getArea(), true, this);

				if (!pWaypointPinch)
				{
					if (distanceFrom(pWaypoint->getOrigin()) > 1024.0)
						pWaypointPinch =
						    CWaypoints::getPinchPointFromWaypoint(pWaypoint->getOrigin(), pWaypoint->getOrigin());
					else
						pWaypointPinch = CWaypoints::getPinchPointFromWaypoint(getOrigin(), pWaypoint->getOrigin());
				}

				if (pWaypointPinch
				    && (distanceFrom(pWaypointPinch->getOrigin()) < distanceFrom(pWaypoint->getOrigin())))
				{
					CBotSchedule *defend = new CBotSchedule();
					CBotTask *findpath1  = new CFindPathTask(pWaypointPinch->getOrigin());
					CBotTask *findpath2  = new CFindPathTask(pWaypoint->getOrigin());
					CBotTask *deftask    = new CBotDefendTask(pWaypointPinch->getOrigin(), randomFloat(3.5f, 6.5f), 0,
					                                          true, pWaypoint->getOrigin(), LOOK_SNIPE);
	...
	case BOT_UTIL_THROW_GRENADE:
	{
		// find hide waypoint
		CWaypoint *pWaypoint =
		    CWaypoints::getWaypoint(CWaypointLocations::GetCoverWaypoint(getOrigin(), m_vLastSeeEnemy, NULL));
		...
		if (pWaypoint)
		{
			...
			pSched->addTask(new CThrowGrenadeTask(pWeapon, getAmmo(pWeapon->getWeaponInfo()->getAmmoIndex1()),
	...
	case BOT_UTIL_ROAM:
	{
		...
		if (pWaypoint)
		{
			if (inSquad() && !isSquadLeader())
			{
				if (pWaypoint->distanceFrom(m_pSquad->GetFormationVector(m_pEdict))
	...
	if (pWeapon != NULL)
	{
		...
		if (pWeapon && pWeapon->isDeployable() && !pWeapon->outOfAmmo(this))
		{
			if (flags & CWaypointTypes::W_FL_MACHINEGUN)
			{
				...
				{
					m_pSchedules->addFront(new CBotSchedule(
					    new CBotDODSnipe(pWeapon, getOrigin(), pWpt->getAimYaw(), false, false, flags)));
		...
		if (!bDontCrouchAndHide && (m_pLastCoverFrom.get() != NULL))
		{
			...
			{
				if (flags
				    & (CWaypointTypes::W_FL_CROUCH | CWaypointTypes::W_FL_MACHINEGUN
				       | CWaypointTypes::W_FL_COVER_RELOAD))
				{
					m_pSchedules->removeSchedule(SCHED_CROUCH_AND_HIDE);
...
bool CDODBot ::handleAttack(CBotWeapon *pWeapon, edict_t *pEnemy)
{
	...
	if (pWeapon)
	{
		...
		if (pWeapon->isMelee()
		    || (pWeapon->isMeleeSecondary() && (pWeapon->outOfAmmo(this) || rcbot_melee_only.GetBool())))
		{
			...
			// enemy below me!
			if (pWeapon->isMelee() && (vEnemyOrigin.z < (getOrigin().z - 8))
		...
		else
		{
			...
			else
			{
				...
				else if (((pWeapon->getID() == DOD_WEAPON_GARAND) || (pWeapon->getID() == DOD_WEAPON_K98))
				         && (distanceFrom(pEnemy) > 1000))
				{
...
{
	return m_pWeapons->hasWeapon(DOD_WEAPON_20CAL) ? m_pWeapons->getWeapon(CWeapons::getWeapon(DOD_WEAPON_20CAL))
...
{
	return m_pWeapons->hasWeapon(DOD_WEAPON_K98_SCOPED)
...
void CDODBot ::getTasks(unsigned int iIgnore)
{
	...
	ADD_UTILITY(
	    ...
	    !hasSomeConditions(CONDITION_DEFENSIVE) && inSquad() && (m_pSquad->GetLeader() != m_pEdict)
	        && (!hasSomeConditions(CONDITION_SEE_SQUAD_LEADER) || !hasSomeConditions(CONDITION_SQUAD_LEADER_INRANGE)),
	    hasSomeConditions(CONDITION_SQUAD_IDLE) ? 0.1f : 2.0f);
	...
	ADD_UTILITY(BOT_UTIL_FIND_LAST_ENEMY,
	            wantToFollowEnemy() && !m_bLookedForEnemyLast && (m_pLastEnemy.get() != NULL)
	...
	if (CDODMod::isFlagMap() && (CDODMod::m_Flags.getNumFlags() > 0) && !rcbot_nocapturing.GetBool())
	{
		...
		if (m_pNearestFlag)
		{
			...
			bAttackNearestFlag = !CDODMod::m_Flags.ownsFlag(iFlagID, m_iTeam)
			                  && ((CDODMod::m_Flags.numCappersRequired(iFlagID, m_iTeam)
			                       - CDODMod::m_Flags.numFriendliesAtCap(iFlagID, m_iTeam))
			                      > 0);
		}
		...
		if (m_pNearestFlag
		    && (!inSquad()
		        || (isSquadLeader()
		            || (hasSomeConditions(CONDITION_SQUAD_IDLE)
		                && (distanceFrom(m_pNearestFlag) < CWaypointLocations::REACHABLE_RANGE)))))
		{
			// attack the flag if I've reached the last one
			ADD_UTILITY_DATA_VECTOR(BOT_UTIL_ATTACK_NEAREST_POINT, bAttackNearestFlag,
			                        (iFlagsOwned == (iNumFlags - 1)) ? 0.9f : 0.75f, iFlagID,
			                        CBotGlobals::entityOrigin(m_pNearestFlag));

			ADD_UTILITY_DATA_VECTOR(BOT_UTIL_DEFEND_NEAREST_POINT,
			                        CDODMod::m_Flags.ownsFlag(iFlagID, m_iTeam)
			                            && ((CDODMod::m_Flags.numEnemiesAtCap(iFlagID, m_iTeam) > 0) || hasEnemy()),
	...
	if (CDODMod::isBombMap() && (CDODMod::m_Flags.getNumFlags() > 0) && !rcbot_nocapturing.GetBool())
	{
		...
		ADD_UTILITY(BOT_UTIL_PICKUP_BOMB, !m_bHasBomb && (iNumBombsToPlant > 0),
		            hasSomeConditions(CONDITION_NEED_BOMB) ? 0.9f : fPlantUtil);

		if (iNumEnemyBombsOnMap > 0)
		{
			ADD_UTILITY(BOT_UTIL_DEFEND_POINT,
			            (iFlagsOwned > 0) && (m_pNearestFlag == NULL) || CDODMod::m_Flags.ownsFlag(iFlagID, m_iTeam),
			            fDefendUtil);
			...
			/*if ( m_pNearestFlag )
			{
			    ...
			    ADD_UTILITY_DATA_VECTOR(BOT_UTIL_ATTACK_NEAREST_POINT,
			        !CDODMod::m_Flags.ownsFlag(iFlagID,m_iTeam) &&
			(CDODMod::m_Flags.numCappersRequired(iFlagID,m_iTeam)-
			        CDODMod::m_Flags.numFriendliesAtCap(iFlagID,m_iTeam))<=1,(iFlagsOwned == (iNumFlags-1)) ? 0.9f :
			0.75f,iFlagID,CBotGlobals::entityOrigin(m_pNearestFlag));
			}*/
		}
		...
		if (m_pNearestBomb
		    && (!inSquad()
		        || (isSquadLeader()
		            || (hasSomeConditions(CONDITION_SQUAD_IDLE)
		                && (distanceFrom(m_pNearestBomb) < CWaypointLocations::REACHABLE_RANGE)))))
		{
			...
			// attack the flag if I've reached the last one
			ADD_UTILITY_DATA_VECTOR(BOT_UTIL_PLANT_NEAREST_BOMB, CDODMod::m_Flags.canPlantBomb(m_iTeam, iFlagID),
			                        fPlantUtil + randomFloat(-0.05f, 0.1f), iFlagID, vBomb);
			// attack the flag if I've reached the last one
			ADD_UTILITY_DATA_VECTOR(BOT_UTIL_DEFEND_NEAREST_BOMB, CDODMod::m_Flags.canDefendBomb(m_iTeam, iFlagID),
			                        fDefendBombUtil + randomFloat(-0.05f, 0.1f), iFlagID, vBomb);
			// attack the flag if I've reached the last one
			ADD_UTILITY_DATA_VECTOR(BOT_UTIL_DEFUSE_NEAREST_BOMB, CDODMod::m_Flags.canDefuseBomb(m_iTeam, iFlagID),
			                        fDefuseBombUtil + randomFloat(-0.05f, 0.1f), iFlagID, vBomb);
		}
	}
	...
	if (!rcbot_melee_only.GetBool() && hasSniperRifle())
	{
		...
		if (pWeapon)
			ADD_UTILITY_WEAPON_DATA(
			    BOT_UTIL_SNIPE, pWeapon->getAmmo(this) > 0,
			    hasSomeConditions(CONDITION_DEFENSIVE) ? 1.6f : (getHealthPercent() * randomFloat(0.75f, 1.0f)),
			    pWeapon, hasSomeConditions(CONDITION_DEFENSIVE) ? false : (fAttackUtil > fDefendUtil));
	}
	else if (!rcbot_melee_only.GetBool() && hasMG())
	{
		...
		if (pWeapon)
			ADD_UTILITY_WEAPON_DATA(BOT_UTIL_SNIPE, pWeapon->getAmmo(this) > 0,
			                        hasSomeConditions(CONDITION_DEFENSIVE) ? 1.6f : (getHealthPercent()), pWeapon,
			                        hasSomeConditions(CONDITION_DEFENSIVE) ? false : (fAttackUtil > fDefendUtil));
	}

	// grenades
	if (!rcbot_melee_only.GetBool() && !hasSomeConditions(CONDITION_SEE_CUR_ENEMY)
	    && hasSomeConditions(CONDITION_SEE_LAST_ENEMY_POS) && m_pLastEnemy && m_fLastSeeEnemy
	    && ((m_fLastSeeEnemy + 10.0) > engine->Time())
	    && (m_pWeapons->hasWeapon(DOD_WEAPON_FRAG_US) || m_pWeapons->hasWeapon(DOD_WEAPON_FRAG_GER)
	        || m_pWeapons->hasWeapon(DOD_WEAPON_SMOKE_US) || m_pWeapons->hasWeapon(DOD_WEAPON_SMOKE_GER)))
	{
		...
		if ((hasSomeConditions(CONDITION_COVERT) || (m_fCurrentDanger >= 25.0f)) && pBotSmokeGren
		    && pBotSmokeGren->hasWeapon())
		...
		if (pBotWeapon && (!pBotWeapon->isExplosive() || (fDistance > BLAST_RADIUS))
		    && (fDistance < (MAX_GREN_THROW_DIST + BLAST_RADIUS)))
		{
                      ...
                      float fDist2D)
{
	...
	if (pWp != NULL)
	{
		if (pWp->isProjectile())
		{
			if (CClassInterface::getVelocity(pEntity, &vel))
				*v_desired_offset = *v_desired_offset
				                  + (vel * randomFloat(m_pProfile->m_fAimSkill - 0.1f, m_pProfile->m_fAimSkill + 0.1f));
			...
			if (pWp->getProjectileSpeed() > 0 && sv_gravity.IsValid())
			{
				...
				v_desired_offset->z =
				    (pow(2, fTime)
				     * (sv_gravity.GetFloat() * rcbot_projectile_tweak.GetFloat())); // - (getOrigin().z - v_origin.z);
			}
	...
	if (m_pNearestSmokeToEnemy)
	{
		...
		if ((iSlot >= 0) && (iSlot < MAX_PLAYERS))
		{
			...
			{
				*v_desired_offset = *v_desired_offset
				                  + Vector(randomFloat(-SMOKE_RADIUS, SMOKE_RADIUS) * smokeinfo->fProb,
				                           randomFloat(-SMOKE_RADIUS, SMOKE_RADIUS) * smokeinfo->fProb,
				                           randomFloat(-SMOKE_RADIUS, SMOKE_RADIUS) * smokeinfo->fProb);
				// return vAim = vAim +
				// Vector(randomFloat(-SMOKE_RADIUS,SMOKE_RADIUS)*smokeinfo->fProb,randomFloat(-SMOKE_RADIUS,SMOKE_RADIUS)*smokeinfo->fProb,randomFloat(-SMOKE_RADIUS,SMOKE_RADIUS)*smokeinfo->fProb);
			}
		}
	}
	...
	if ((iSlot >= 0) && (iSlot < MAX_PLAYERS))
	{
		...
		if (smokeinfo->fLastTime < engine->Time())
		{
			...
			if (CClients::clientsDebugging(BOT_DEBUG_THINK))
				CClients::clientDebugMsg(this, BOT_DEBUG_THINK, "Smoke Test (%s to %s) = %s", m_szBotName,
				                         engine->GetPlayerNetworkIDString(pCheck),
				                         smokeinfo->bVisible ? "visible" : "invisible");
#endif
		}
