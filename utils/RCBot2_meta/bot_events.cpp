...
class CBotHearPlayerAttack : public IBotFunction
{
  public:
	...
	void execute(CBot *pBot)
	{
		if (!pBot->hasEnemy() && (pBot->wantToListen() || pBot->isListeningToPlayer(m_pAttacker))
		    && pBot->wantToListenToPlayerAttack(m_pAttacker, m_iWeaponID))
		{
...
void CPlayerDeathEvent ::execute(IBotEventInterface *pEvent)
{
	...
	if (pAttacker
	    && ((CBotGlobals::entityOrigin(pAttacker) - CBotGlobals::entityOrigin(m_pActivator)).Length() > 512.0f))
	{
		// killer
...
// non OO hack here
		...
		if (CBotGlobals::isPlayer(pAttacker) && pClient && pClient->autoWaypointOn())
		{
			...
			if (pWeapon != nullptr)
			{
				if (pWeapon->isScoped())
				{
					pClient->autoEventWaypoint(CWaypointTypes::W_FL_SNIPER, 200.0f, true,
					                           CClassInterface::getTeam(pAttacker),
					                           CBotGlobals::entityOrigin(pAttacker) + Vector(0, 0, 32.0f));
				}
			}
		}
	}
 ...
 */
	...
	CBotTF2FunctionEnemyAtIntel fn(capteam, CTeamFortress2Mod::m_ObjectiveResource.GetCPPosition(capindex),
	                               EVENT_CAPPOINT, nullptr, capindex);
	CBots::botFunction(&fn);
}
