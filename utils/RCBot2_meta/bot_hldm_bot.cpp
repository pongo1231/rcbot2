...
bool CHLDMBot ::executeAction(eBotAction iAction)
{
	...
	case BOT_UTIL_SNIPE:
	{
		...
		if (pWaypoint)
		{
			...
			// use DOD task
			snipetask = new CBotHL2DMSnipe(m_pWeapons->getWeapon(CWeapons::getWeapon(HL2DM_WEAPON_CROSSBOW)),
...
void CHLDMBot ::getTasks(unsigned int iIgnore)
{
		...
		if (CBotGlobals::entityIsValid(pent))
		{
			ADD_UTILITY(
			    ...
			    (!m_pEnemy || (m_pCurrentWeapon && (strcmp("weapon_physcannon", m_pCurrentWeapon->GetClassName()))))
			        && gravgun && gravgun->hasWeapon() && (m_NearestPhysObj.get() != NULL)
			        && (gravgun->getWeaponIndex() > 0)
			        && (CClassInterface::gravityGunObject(INDEXENT(gravgun->getWeaponIndex())) == NULL),
			    0.9f);
		}
	}
	...
	ADD_UTILITY(BOT_UTIL_FIND_NEAREST_HEALTH, (m_pHealthKit.get() != NULL) && (getHealthPercent() < 1.0f),
	            1.0f - getHealthPercent());

	// low on armor?
	ADD_UTILITY(BOT_UTIL_HL2DM_FIND_ARMOR, (m_pBattery.get() != NULL) && (getArmorPercent() < 1.0f),
	            (1.0f - getArmorPercent()) * 0.75f);
	ADD_UTILITY(BOT_UTIL_HL2DM_USE_CHARGER,
	            (m_pCharger.get() != NULL) && (CClassInterface::getAnimCycle(m_pCharger) < 1.0f)
	    ...
	    && m_fLastSeeEnemy && ((m_fLastSeeEnemy + 10.0) > engine->Time()) && m_pWeapons->hasWeapon(HL2DM_WEAPON_FRAG))
	{
		...
		if ((fDistance > BLAST_RADIUS) && (fDistance < 1500))
		{
			...
			ADD_UTILITY(BOT_UTIL_THROW_GRENADE, pBotWeapon && (pBotWeapon->getAmmo(this) > 0),
			            1.0f - (getHealthPercent() * 0.2));
		}
	}
...
void CHLDMBot ::modThink()
{
	...
	if ((m_fCurrentDanger >= 20.0f) && (CClassInterface::auxPower(m_pEdict) > 90.f)
	    && (m_flSprintTime < engine->Time()))
	{
