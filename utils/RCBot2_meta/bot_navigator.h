...
class IBotNavigator
{
  public:
	...
	virtual bool workRoute(Vector vFrom, Vector vTo, bool *bFail, bool bRestart = true, bool bNoInterruptions = false,
	                       int iGoalId = -1, int iConditions = 0, int iDangerId = -1) = 0;
	...
	inline Vector getPreviousPoint()
	{
		return m_vPreviousPoint;
	}
	...
	virtual int getCurrentFlags()
	{
		return 0;
	}
	virtual int getPathFlags(int iPath)
	{
		return 0;
	}
	...
	virtual bool nextPointIsOnLadder()
	{
		return false;
	}

	virtual bool beliefLoad()
	{
		return false;
	};

	virtual bool beliefSave(bool bOverride = false)
	{
		return false;
	};
	...
	virtual float getCurrentBelief()
	{
		return 0;
	}

	virtual float getBelief(int index)
	{
		return 0;
	}
	...
	virtual bool randomDangerPath(Vector *vec)
	{
		return false;
	}

	bool getDangerPoint(Vector *vec)
	{
		*vec = m_bDangerPoint ? m_vDangerPoint : Vector(0, 0, 0);
		return m_bDangerPoint;
	}

	bool wantToLoadBelief()
	{
		return m_bLoadBelief;
	}
	virtual bool wantToSaveBelief()
	{
		return false;
	}
...
class AStarNode
{
  public:
	...
	inline void close()
	{
		setFlag(FL_ASTAR_CLOSED);
	}
	inline void unClose()
	{
		removeFlag(FL_ASTAR_CLOSED);
	}
	inline bool isOpen()
	{
		return hasFlag(FL_ASTAR_OPEN);
	}
	inline void unOpen()
	{
	...
	{
		return hasFlag(FL_ASTAR_CLOSED);
	}
	inline void open()
	{
		setFlag(FL_ASTAR_OPEN);
	}
	//////////////////////////////////////////////////////
	inline void setHeuristic(float fHeuristic)
	{
		m_fHeuristic = fHeuristic;
		setFlag(FL_HEURISTIC_SET);
	}
	inline bool heuristicSet()
	{
		return hasFlag(FL_HEURISTIC_SET);
	}
	inline const float getHeuristic()
	{
		return m_fHeuristic;
	}
	const
	...
	////////////////////////////////////////////////////////
	inline const float getCost()
	{
		return m_fCost;
	}
	const inline void setCost(float fCost)
	{
		m_fCost = fCost;
	}
	////////////////////////////////////////////////////////
	...
	void setWaypoint(int iWpt)
	{
		m_iWaypoint = iWpt;
	}
	inline int getWaypoint()
	{
		return m_iWaypoint;
	}
...
class CWaypointNavigator : public IBotNavigator
{
  public:
	...
	CWaypoint *chooseBestFromBelief(std::vector<CWaypoint *> &goals, bool bHighDanger = false, int iSearchFlags = 0,
	                                int iTeam = 0);
	CWaypoint *chooseBestFromBeliefBetweenAreas(std::vector<AStarNode *> &goals, bool bHighDanger = false,
	                                            bool bIgnoreBelief = false);
	...
	void getFailedGoals(WaypointList **goals)
	{
		*goals = &m_iFailedGoals;
	}
