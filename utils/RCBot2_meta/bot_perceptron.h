...
class CNeuron
{
  public:
	...
	inline ga_nn_value getWeight(unsigned short int i)
	{
		return m_weights[i];
	}
...
class CLogisticalNeuron : public CNeuron
{
  public:
	...
	inline void setError(ga_nn_value err)
	{
		m_error = err;
	}
	inline void addError(ga_nn_value err)
	{
		m_error += err;
	}
	inline void divError(unsigned short int samples)
	{
		m_error /= samples;
	}

	inline ga_nn_value getError(unsigned short int w)
	{
		return m_error * m_weights[w];
	}
	inline ga_nn_value getMSE()
	{
		return m_error;
	}
...
class CTrainingSet
{
  public:
	...
	inline ga_nn_value getMinScale()
	{
		return m_fMin;
	}
	inline ga_nn_value getMaxScale()
	{
		return m_fMax;
	}

	inline training_batch_t *getBatches()
	{
		return batches;
	}
